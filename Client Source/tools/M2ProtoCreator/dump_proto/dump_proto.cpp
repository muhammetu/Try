...
bool Set_Proto_Mob_Table(CPythonNonPlayer::TMobTable *mobTable, cCsvTable &csvTable, std::map<int,const char*> &nameMap)
{
	...
	mobTable->dwMaxHP              = atoll(csvTable.AsStringByIndex(col++));
	mobTable->bRegenCycle          = atoi(csvTable.AsStringByIndex(col++));
	mobTable->bRegenPercent        = atoi(csvTable.AsStringByIndex(col++));

	mobTable->llGoldMin            = atoll(csvTable.AsStringByIndex(col++));
	mobTable->llGoldMax            = atoll(csvTable.AsStringByIndex(col++));
			...
			else
			{
				...
				for (DWORD i = 0; i < tableSize; ++i)
				{
					...
					fprintf(mf2,
						...
						rTable.dwDamageRange[0], rTable.dwDamageRange[1], rTable.dwMaxHP, rTable.bRegenCycle, rTable.bRegenPercent, rTable.llGoldMin, rTable.llGoldMax, rTable.dwExp, rTable.wDef,
...
bool Set_Proto_Item_Table(CItemData::TItemTable *itemTable, cCsvTable &csvTable, std::map<int,const char*> &nameMap)
{
	...
	itemTable->llIBuyItemPrice = atoll(csvTable.AsStringByIndex(col++));
	itemTable->llISellItemPrice = atoll(csvTable.AsStringByIndex(col++));
	itemTable->dwRefinedVnum = atoi(csvTable.AsStringByIndex(col++));
	itemTable->dwRefineSet = atoi(csvTable.AsStringByIndex(col++));
...
bool BuildItemTable()
{
	...
	while (data.Next())
	{
		...
		if(it_map_itemTable == test_map_itemTableByVnum.end()) {
			...
			item_table->llIBuyItemPrice = tempTable->llIBuyItemPrice;
			item_table->llISellItemPrice = tempTable->llISellItemPrice;
			item_table->dwRefinedVnum = tempTable->dwRefinedVnum;
			item_table->dwRefineSet = tempTable->dwRefineSet;
			...
			else
			{
				...
				for (DWORD i = 0; i < tableSize; ++i)
				{
					...
					fprintf(mf2,
						...
						rTable.llIBuyItemPrice, rTable.llISellItemPrice, rTable.dwRefinedVnum, rTable.dwRefineSet, rTable.bAlterToMagicItemPct, // 14
