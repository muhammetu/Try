...
const char* GetLowerString(const char* String)
{
	static char szString[1024];
	strcpy_s(szString, String);
	StringLowers(szString);
	return szString;
}
...
#include <fmt/fmt.h>
#include <fmt/core.h>
std::string convertTime(DWORD seconds)
{
	if (seconds < 60)
		return fmt::format("{} Saniye", seconds);

	auto minutes = (seconds / 60) % 60;
	auto hours = ((seconds / 60) / 60) % 24;
	auto days = (((seconds / 60) / 60) / 24);

	std::string ret = "";
	if (days)
		ret += fmt::format("{} Gün ", days);
	if (hours)
		ret += fmt::format("{} Saat ", hours);
	if (minutes)
		ret += fmt::format("{} Dakika ", minutes);
