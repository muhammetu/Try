...
void CMappedFile::Link(DWORD dwBufSize, const void* c_pvBufData, bool transferOwnership)
{
	if (m_isOwner)
	{
		delete[] m_pbBufLinkData;
	}
	m_isOwner = transferOwnership;
...
void CMappedFile::Destroy()
{
	...
	if (m_pbBufLinkData && m_dwBufLinkSize > 0 && m_isOwner)
	{
		delete[] m_pbBufLinkData;
	}
	m_dwAppendResultDataSize = 0;
