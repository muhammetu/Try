...
bool CActorInstance::__CanPushDestActor(CActorInstance& rkActorDst)
{
	...
	// @duzenleme
	// bosslar firlatmasin diye yapilmis.
	if (rkActorDst.IsBoss())
		return false;
...
void CActorInstance::__ProcessDataAttackSuccess(const NRaceData::TAttackData& c_rAttackData, CActorInstance& rVictim, const D3DXVECTOR3& c_rv3Position, UINT uiSkill, BOOL isSendPacket)
{
	...
	if (rVictim.IsStone() || rVictim.IsDoor())
	{
		__HitStone(rVictim);
	}
	else
	{
		...
		// Motion
		else if (NRaceData::HIT_TYPE_GOOD == c_rAttackData.iHittingType || rVictim.IsResistFallen())
		{
			__HitGood(rVictim);
		}
		else if (NRaceData::HIT_TYPE_GREAT == c_rAttackData.iHittingType)
		{
			if (IsBoss() && rVictim.IsPC())
				__HitGood(rVictim);
			else
				__HitGreate(rVictim);
		}
...
float CActorInstance::__GetInvisibleTimeAdjust(const UINT uiSkill, const NRaceData::TAttackData& c_rAttackData){
	static const int shamanw = 3, shamanm = 7, wolfman = 8, warriorm = 0, warriorw = 4, suram = 2, suraw = 6, ninjaw = 5, ninjam = 1;

	if ((GetRace() != shamanw && GetRace() != shamanm && GetRace() != wolfman && GetRace() != warriorm && GetRace() != warriorw && GetRace() != suram && GetRace() != suraw && GetRace() != ninjaw && GetRace() != ninjam) ||
		uiSkill != 0 ||
		m_fAtkSpd < 1.3)
		return 0.0f;
