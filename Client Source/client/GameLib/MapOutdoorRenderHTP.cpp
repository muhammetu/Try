...
void CMapOutdoor::__HardwareTransformPatch_RenderPatchSplat(long patchnum, WORD wPrimitiveCount, D3DPRIMITIVETYPE ePrimitiveType)
{
	...
	if (CPythonSystem::instance().GetSnowTextureModeOption())
	{
		...
		for (DWORD j = 1; j < pTerrain->GetNumSnowTextures(); ++j)
		{
			...
			auto aIterator = std::find(m_RenderedTextureNumVector.begin(), m_RenderedTextureNumVector.end(), static_cast<int>(j));
	...
	else if (CPythonSystem::instance().GetDesertTextureModeOption())
	{
		bool isFirst = true;
		for (DWORD j = 1; j < pTerrain->GetNumDesertTextures(); ++j)
		{
			TTerainSplat & rSplat = rTerrainSplatPatch.Splats[j];
			...
			const TTerrainTexture & rTexture = m_DesertTextureSet.GetTexture(j);
