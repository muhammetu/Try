	...
	for (itor = m_DefendingPointInstanceList.begin(); itor != m_DefendingPointInstanceList.end(); ++itor)
	{
		...
		for (DWORD i = 0; i < c_pAttackingSphereVector->size(); ++i)
			for (DWORD j = 0; j < c_pDefendingSphereVector->size(); ++j)
			{
				...
				{
					float fActorHitRange = CPythonNonPlayer::Instance().GetMonsterHitRange(GetRace());

					D3DXVECTOR3 rv3DestPos = c_rAttackingSphere.v3Position;
					D3DXVECTOR3 rv3SelfPos = GetPosition();

					rv3SelfPos.z += (GetScale().z - 1.0f) * (fActorHitRange / GetScale().z);

					D3DXVECTOR3 rv3Distance = rv3DestPos - rv3SelfPos;
...
bool CActorInstance::CreateCollisionInstancePiece(DWORD dwAttachingModelIndex, const NRaceData::TAttachingData* c_pAttachingData, TCollisionPointInstance* pPointInstance)
{
	...
	for (it = c_rSphereDataVector.begin(); it != c_rSphereDataVector.end(); ++it)
	{
		...
		dsi.fRadius = c_rSphereData.fRadius * GetScale().z;
...
BOOL CActorInstance::__NormalAttackProcess(CActorInstance& rVictim)
{
	...
	/*else if (rVictim.IsStone())
	{
		if (fDistance >= 300.0f * 750.0f)
			return FALSE;
	}*/
	else
	{
...
{
	// @duzenleme
	// collision checki safezone npc ve enemy olarak siniflandirilmis.
	if (IsInSafe(rVictim))
		return FALSE;

	if (rVictim.IsNPC() && !rVictim.IsDungeonNPC() && !rVictim.IsBoss())
		return FALSE;
