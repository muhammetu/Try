...
void __GetRaceResourcePathes(unsigned race, std::vector <std::string> & vec_stPathes)
{
	...
	else if (__IsNPCRace(race))
	{
		...
		{
			if ((race >= 20233 && race <= 20299) || (race >= 20945 && race <= 20957) || (race >= 9920 && race <= 9923) || race == 9928 || (race >= 9598 && race <= 9599)) // last known 20247
			{
				vec_stPathes.push_back("d:/ymir work/npc_mount/");
				vec_stPathes.push_back("d:/ymir work/npc/");
			}
...
bool CRaceManager::RaceIsAvaible(DWORD dwRaceIndex)
{
	return false;
}
...
bool CRaceManager::__LoadRaceMotionList(CRaceData& rkRaceData, const char* pathName, const char* motionListFileName)
{
	...
	for (UINT uLineIndex=0; uLineIndex<uLineCount; ++uLineIndex)
	{
		...
		rkRaceData.RegisterMotionData(CRaceMotionData::MODE_GENERAL, motionType, stMotionFileName.c_str(), nPercent, true);
	...
	if (!isSpawn && stSpawnMotionFileName != "")
		rkRaceData.RegisterMotionData(CRaceMotionData::MODE_GENERAL, CRaceMotionData::NAME_SPAWN, stSpawnMotionFileName.c_str(), nPercent, true);
