...
bool CDragonSoulTable::LoadVnumMapper()
{
	...
	while (TextFileLoader.GetTokenVector(std::to_string(++id), &values))
	{
		...
		uint8_t iType = atoi(values->at(1).c_str());
...
bool CDragonSoulTable::LoadWeightTable()
{
	...
	for (uint8_t iGrade = 0; iGrade < DRAGON_SOUL_GRADE_MAX; ++iGrade)
	{
		...
		for (uint8_t iStep = 0; iStep < DRAGON_SOUL_STEP_MAX; ++iStep)
		{
			...
			for (uint8_t iStrength = 0; iStrength < DRAGON_SOUL_STRENGTH_MAX; ++iStrength)
			{
...
uint8_t CDragonSoulTable::GetDSSetWeight(uint8_t iSetGrade)
{
...
uint8_t CDragonSoulTable::GetDSBasicApplyCount(uint8_t iDSType)
{
	...
	return (uint8_t)it->second.vec_BasicApplys.size();
}
uint16_t CDragonSoulTable::GetDSBasicApplyValue(uint8_t iDSType, uint8_t applyType)
{
	const auto vnumMapperIt = m_map_VnumMapper.find(iDSType);
	if (vnumMapperIt == m_map_VnumMapper.end())
	{
...
uint16_t CDragonSoulTable::GetDSAdditionalApplyValue(uint8_t iDSType, uint8_t applyType)
{
	const auto vnumMapperIt = m_map_VnumMapper.find(iDSType);
	if (vnumMapperIt == m_map_VnumMapper.end())
	{
...
uint16_t CDragonSoulTable::GetApplyType(std::string applyName)
{
	struct SValueName
	{
		std::string stName;
	...
	SValueName ApplyTypeNames[] =
	{
		...
		{ "ATTBONUS_BOSS",CItemData::APPLY_ATTBONUS_BOSS },
		{ "ATTBONUS_STONE",CItemData::APPLY_ATTBONUS_STONE },
		{ "ATTBONUS_ELEMENTS",CItemData::APPLY_ATTBONUS_ELEMENTS },
		{ "ENCHANT_ELEMENTS",CItemData::APPLY_ENCHANT_ELEMENTS },
		{ "ATTBONUS_CHARACTERS",CItemData::APPLY_ATTBONUS_CHARACTERS },
		{ "ENCHANT_CHARACTERS",CItemData::APPLY_ENCHANT_CHARACTERS },
#endif
#ifdef ENABLE_CHEST_DROP_POINT
		{ "CHEST_BONUS",CItemData::APPLY_CHEST_BONUS },
#endif
	};
