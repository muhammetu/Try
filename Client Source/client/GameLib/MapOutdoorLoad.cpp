...
bool CMapOutdoor::LoadSetting(const char* c_szFileName)
{
	...
	std::string stSnowTextureSetFileName = "textureset\\snow\\snow_metin2_a1.txt";
	std::string stDesertTextureSetFileName = "textureset\\desert\\desert_metin2_a1.txt";
#endif
	...
	if (!m_DesertTextureSet.Load(stDesertTextureSetFileName.c_str(), m_fTerrainTexCoordBase))
	{
		TraceError("%s Could not load ", stDesertTextureSetFileName.c_str());
		m_DesertTextureSet.Load(stTextureSetFileName.c_str(), m_fTerrainTexCoordBase);
	}
#endif
	...
	CTerrain::SetDesertTextureSet(&m_DesertTextureSet);
#endif
