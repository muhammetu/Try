...
#endif
{
	...
	if (c_pEffectData->isAttaching)
	{
		...
		{
			AttachEffectByID(0, c_pEffectData->strAttachingBoneName.c_str(), c_pEffectData->dwEffectIndex, &c_pEffectData->v3EffectPosition, dwSkillColor, false, c_v3Scale.x, &c_v3Scale);
		}
		else
		{
			int iBoneIndex;
	...
	{
#ifdef ENABLE_SKILL_COLOR_SYSTEM
		AttachEffectByID(0, NULL, c_pEffectData->dwEffectIndex, &c_pEffectData->v3EffectPosition, dwSkillColor, false, c_v3Scale.x, &c_v3Scale);
#else
	...
	if (m_kFlyTarget.IsValidTarget())
	{
		...
		if (m_eRace == 1 || m_eRace == 5)
		{
			...
			if (NRaceData::MOTION_TYPE_NORMAL == c_pData->iMotionType)
			{
				if (IsEqippedQuiver())
#ifdef ENABLE_SKILL_COLOR_SYSTEM
					pInstance = rfm.CreateIndexedFlyingInstanceFlyTarget(GetQuiverEffectID(), v3Start, m_kFlyTarget, dwSkillColor);
#else
