...
PyObject* playerGetMainCharacterID(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonPlayer::Instance().GetPlayerID());
}
...
PyObject* playerGetRankData(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	int iRankPoint = 0;
	if (pInstance)
		iRankPoint = pInstance->GetRank();
	return Py_BuildValue("i", iRankPoint);
}
#endif

#ifdef ENABLE_TEAM_SYSTEM
PyObject* playerGetTeamData(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	int iRankPoint = 0;
	if (pInstance)
		iRankPoint = pInstance->GetTeam();
	return Py_BuildValue("i", iRankPoint);
}
#endif

#ifdef ENABLE_LANDRANK_SYSTEM
PyObject* playerGetLandRankData(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	int iLandRankPoint = 0;
	if (pInstance)
		iLandRankPoint = pInstance->GetLandRank();
	return Py_BuildValue("i", iLandRankPoint);
}
PyObject* playerGetLandRankString(PyObject* poSelf, PyObject* poArgs)
{
	string str = "";
	int rank;

	if (!PyTuple_GetInteger(poArgs, 0, &rank))
		return Py_BuildException();

	CInstanceBase::GetLandRankString(str, rank);
...
PyObject* playerGetRebornData(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	int iRebornPoint = 0;
	if (pInstance)
		iRebornPoint = pInstance->GetReborn();
	return Py_BuildValue("i", iRebornPoint);
}
#endif

#ifdef ENABLE_MONIKER_SYSTEM
PyObject* playerGetMoniker(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();

	return Py_BuildValue("s", pInstance ? pInstance->GetMoniker() : "");
}
#endif

#ifdef ENABLE_LOVE_SYSTEM
PyObject* playerGetLove1(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();

	return Py_BuildValue("s", pInstance ? pInstance->GetLove1() : "");
}

PyObject* playerGetLove2(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();

	return Py_BuildValue("s", pInstance ? pInstance->GetLove2() : "");
}
#endif // ENABLE_LOVE_SYSTEM

#ifdef ENABLE_WORD_SYSTEM
PyObject* playerGetWord(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();

	return Py_BuildValue("s", pInstance ? pInstance->GetWord() : "");
}
...
PyObject* playerGetItemEvolutionType(PyObject* poSelf, PyObject* poArgs)
{
	int iEvolution;
	if (!PyTuple_GetInteger(poArgs, 0, &iEvolution))
		return Py_BadArgument();

	else if (iEvolution > 300)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_EX_ALL);
	else if (iEvolution > 280)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_EX_ELEC);
	else if (iEvolution > 260)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_EX_DARK);
	else if (iEvolution > 240)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_EX_WIND);
	else if (iEvolution > 220)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_EX_EARTH);
	else if (iEvolution > 200)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_EX_ICE);
	else if (iEvolution > 180)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_EX_FIRE);
	else if (iEvolution > 160)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_HUMAN);
	else if (iEvolution > 140)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_ALL);
	else if (iEvolution > 120)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_ELEC);
	else if (iEvolution > 100)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_DARK);
	else if (iEvolution > 80)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_WIND);
	else if (iEvolution > 60)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_EARTH);
	else if (iEvolution > 40)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_ICE);
	else if (iEvolution > 20)
		return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_FIRE);

	return Py_BuildValue("i", WEAPON_EVOLUTION_TYPE_NONE);
}

PyObject* playerGetItemRealEvolution(PyObject* poSelf, PyObject* poArgs)
{
	int iEvolution;
	if (!PyTuple_GetInteger(poArgs, 0, &iEvolution))
		return Py_BadArgument();
	...
	if (iEvolution > 300)
		return Py_BuildValue("i", iEvolution - 300);
	else if (iEvolution > 280)
		return Py_BuildValue("i", iEvolution - 280);
	else if (iEvolution > 260)
		return Py_BuildValue("i", iEvolution - 260);
	else if (iEvolution > 240)
		return Py_BuildValue("i", iEvolution - 240);
	else if (iEvolution > 220)
		return Py_BuildValue("i", iEvolution - 220);
	else if (iEvolution > 200)
		return Py_BuildValue("i", iEvolution - 200);
	else if (iEvolution > 180)
		return Py_BuildValue("i", iEvolution - 180);
	else if (iEvolution > 160)
		return Py_BuildValue("i", iEvolution - 160);
	else if (iEvolution > 140)
		return Py_BuildValue("i", iEvolution - 140);
	else if (iEvolution > 120)
		return Py_BuildValue("i", iEvolution - 120);
	else if (iEvolution > 100)
		return Py_BuildValue("i", iEvolution - 100);
	else if (iEvolution > 80)
		return Py_BuildValue("i", iEvolution - 80);
	else if (iEvolution > 60)
		return Py_BuildValue("i", iEvolution - 60);
	else if (iEvolution > 40)
		return Py_BuildValue("i", iEvolution - 40);
	else if (iEvolution > 20)
		return Py_BuildValue("i", iEvolution - 20);

	return Py_BuildValue("i", 0);
}

PyObject* playerGetItemEvolutionAtk(PyObject* poSelf, PyObject* poArgs)
{
	int iEvolutionType;
	if (!PyTuple_GetInteger(poArgs, 0, &iEvolutionType))
		return Py_BadArgument();
	...
	if (iEvolutionType == WEAPON_EVOLUTION_TYPE_HUMAN)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 160;
		return Py_BuildValue("i", AtkPoints[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_ALL)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 140;
		return Py_BuildValue("i", AtkPoints[tempEvolutionVal]*2);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_ELEC)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 120;
		return Py_BuildValue("i", AtkPoints[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_DARK)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 100;
		return Py_BuildValue("i", AtkPoints[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_WIND)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 80;
		return Py_BuildValue("i", AtkPoints[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_EARTH)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 60;
		return Py_BuildValue("i", AtkPoints[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_ICE)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 40;
		return Py_BuildValue("i", AtkPoints[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_FIRE)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 20;
		return Py_BuildValue("i", AtkPoints[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_EX_FIRE)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 180;
		return Py_BuildValue("i", AtkPointsEx[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_EX_ICE)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 200;
		return Py_BuildValue("i", AtkPointsEx[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_EX_EARTH)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 220;
		return Py_BuildValue("i", AtkPointsEx[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_EX_WIND)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 240;
		return Py_BuildValue("i", AtkPointsEx[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_EX_DARK)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 260;
		return Py_BuildValue("i", AtkPointsEx[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_EX_ELEC)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 280;
		return Py_BuildValue("i", AtkPointsEx[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_EX_ALL)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 300;
		return Py_BuildValue("i", AtkPointsExEl[tempEvolutionVal]);
	}

	return Py_BuildValue("i", 0);
}

PyObject* playerGetItemEvolutionElementAtk(PyObject* poSelf, PyObject* poArgs)
{
	int iEvolutionType;
	if (!PyTuple_GetInteger(poArgs, 0, &iEvolutionType))
		return Py_BadArgument();
	...
	if (iEvolutionType == WEAPON_EVOLUTION_TYPE_ELEC)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 120;
		return Py_BuildValue("i", ElementPoints[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_DARK)
	{
		int tempEvolutionVal = iEvolution;
	...
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_ALL)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 140;
		return Py_BuildValue("i", ElementPoints[tempEvolutionVal]*2);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_HUMAN)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 160;
		return Py_BuildValue("i", ElementPoints[tempEvolutionVal]);
	...
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_EX_FIRE)
	{
		int tempEvolutionVal = iEvolution;
		tempEvolutionVal -= 180;
		return Py_BuildValue("i", ElementPointsEx[tempEvolutionVal]);
	}
	else if (iEvolutionType == WEAPON_EVOLUTION_TYPE_EX_ICE)
	{
		int tempEvolutionVal = iEvolution;
...
PyObject* playerGetItemEvolutionArmorElementAtk(PyObject* poSelf, PyObject* poArgs)
{
	int iEvolutionType;
	if (!PyTuple_GetInteger(poArgs, 0, &iEvolutionType))
		return Py_BadArgument();
...
PyObject* playerGetItemEvolutionArmorAtk(PyObject* poSelf, PyObject* poArgs)
{
	int iEvolutionType;
	if (!PyTuple_GetInteger(poArgs, 0, &iEvolutionType))
		return Py_BadArgument();
...
PyObject* playerSetItemCount(PyObject* poSelf, PyObject* poArgs)
{
	...
	case 2:
	{
		...
		BYTE bCount;
#endif
		if (!PyTuple_GetUnsignedLong(poArgs, 1, &bCount))
			return Py_BuildException();
	...
	case 3:
	{
		...
		BYTE bCount;
#endif
		if (!PyTuple_GetUnsignedLong(poArgs, 2, &bCount))
			return Py_BuildException();
...
#ifdef ENABLE_ITEM_EVOLUTION_SYSTEM
		len = snprintf(itemlink, sizeof(itemlink), "item:%x:%x:%x:%x:%x:%x:%d",
			pPlayerItem->vnum, pPlayerItem->flags,
			pPlayerItem->alSockets[0], pPlayerItem->alSockets[1], pPlayerItem->alSockets[2], pPlayerItem->alSockets[3], pPlayerItem->evolution);
#else
...
#ifdef ENABLE_ITEM_ENTITY_UTILITY
		len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%d", pPlayerItem->count);
#endif

#ifdef ENABLE_CHANGELOOK_SYSTEM
		len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%d", pPlayerItem->transmutation);
#endif
...
#ifdef ENABLE_GOLD_LIMIT_REWORK
	long long llValue = pItemData->GetISellItemPrice();
	if (llValue > 5000)
		isHighPrice = TRUE;
#else
...
PyObject* playerCanDetach(PyObject* poSelf, PyObject* poArgs)
{
	...
	if ((iTargetType == CItemData::ITEM_TYPE_ARMOR && (iTargetSubType == CItemData::ARMOR_WRIST || iTargetSubType == CItemData::ARMOR_NECK || iTargetSubType == CItemData::ARMOR_EAR || iTargetSubType == CItemData::ARMOR_HEAD || iTargetSubType == CItemData::ARMOR_SHIELD || iTargetSubType == CItemData::ARMOR_FOOTS || iTargetSubType == CItemData::ARMOR_PENDANT)) || (iTargetType == CItemData::ITEM_TYPE_BELT))
#else
	if ((iTargetType == CItemData::ITEM_TYPE_ARMOR && (iTargetSubType == CItemData::ARMOR_WRIST || iTargetSubType == CItemData::ARMOR_NECK || iTargetSubType == CItemData::ARMOR_EAR)) || (iTargetType == CItemData::ITEM_TYPE_BELT))
#endif // ENABLE_NEW_ACCESORY_ITEMS
	{
		if (pScrollItemData->GetIndex() != 25100)
			return Py_BuildValue("i", DETACH_METIN_CANT);

		int curCount = CPythonPlayer::Instance().GetItemMetinSocket(TargetSlotIndex, 0);
		// int maxCount = CPythonPlayer::Instance().GetItemMetinSocket(TargetSlotIndex, 1);

		return Py_BuildValue("i", curCount > 0);
	}
...
PyObject* playerGetPartyMemberAffects(PyObject* poSelf, PyObject* poArgs)
{
	...
	return Py_BuildValue("iiiiiiiiii", pPartyMemberInfo->sAffects[0],
#else
...
#ifdef ENABLE_PARTY_ROLE_REWORK
		, pPartyMemberInfo->sAffects[7]
		, pPartyMemberInfo->sAffects[8]
		, pPartyMemberInfo->sAffects[9]
#endif
	);
}
...
PyObject* playerGetSkillColor(PyObject* poSelf, PyObject* poArgs)
{
	...
	return Py_BuildValue("iiiii", dwSkillColor[0], dwSkillColor[1], dwSkillColor[2], dwSkillColor[3], dwSkillColor[4]); //TODO: Yeter.
}
#endif

#ifdef ENABLE_UNBUG_CHARACTER_SYSTEM
PyObject* playerUnBugCharacter(PyObject* poSelf, PyObject* poArgs)
{
	CInstanceBase* pMainInstance = CPythonPlayer::Instance().NEW_GetMainActorPtr();
	if (pMainInstance)
	{
		...
		yBuldum:
			if (gyBuldum) {
				...
				CPythonChat::Instance().AppendChat(1, XOR("<Bugdan Kurtul>  Karakteriniz bloklu yerden kurtarildi."));
				return Py_BuildValue("i", 1);
			}
			else {
				CPythonChat::Instance().AppendChat(1, XOR("<Bugdan Kurtul> Sistem karakterinizi kurtaramadi. Karakter atmayi deneyin."));
			}
		}
	}
	CPythonChat::Instance().AppendChat(1, XOR("<Bugdan Kurtul> Karakteriniz bloklu degil."));
	return Py_BuildValue("i", 0);
}
#endif

// @duzenleme
// tr den alinan gruptan atamama fixi.
...
PyObject* playerUseItemByVnum(PyObject* poSelf, PyObject* poArgs)
{
	DWORD dwVnum = 0;
	...
	return Py_BuildValue("i", CPythonPlayer::Instance().UseItemByVnum(dwVnum));
}
#endif


#ifdef ENABLE_AUTO_HUNT_SYSTEM
PyObject * playerOtoAvSaldiriAta(PyObject* poSelf, PyObject* poArgs){
	int gelenDurum;
	if (!PyTuple_GetInteger(poArgs, 0, &gelenDurum)){ return Py_BuildNone(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvSaldiriAta(gelenDurum);
	return Py_BuildNone();
}
PyObject * playerOtoAvOdakAta(PyObject* poSelf, PyObject* poArgs){
	int gelenDurum;
	if (!PyTuple_GetInteger(poArgs, 0, &gelenDurum)){ return Py_BuildNone(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	rkPlayer.OtoAvOdakAta(gelenDurum);
	return Py_BuildNone();
}
PyObject * playerOtoAvSaldiriAl(PyObject* poSelf, PyObject* poArgs){
	int gelenDurum;
	if (!PyTuple_GetInteger(poArgs, 0, &gelenDurum)){ return Py_BuildNone(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	return Py_BuildValue("i", rkPlayer.OtoAvSaldiriAl());
}
PyObject * playerOtoAvOdakAl(PyObject* poSelf, PyObject* poArgs){
	int gelenDurum;
	if (!PyTuple_GetInteger(poArgs, 0, &gelenDurum)){ return Py_BuildNone(); }
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	return Py_BuildValue("i", rkPlayer.OtoAvOdakAl());
}
PyObject * playerOtoAvDurumAta(PyObject* poSelf, PyObject*poArgs){
	bool gelenDurum;
	if (!PyTuple_GetBoolean(poArgs, 0, &gelenDurum)){ return Py_BuildValue("i", 0); }
	/**/
	CInstanceBase * pInstance = CPythonCharacterManager::Instance().GetSelectedInstancePtr();
	CPythonPlayer& rkPlayer = CPythonPlayer::Instance();
	if (pInstance){
		if (rkPlayer.OtoAvDurumAl() != gelenDurum){
			rkPlayer.OtoAvDurumAta(gelenDurum);
			return Py_BuildNone();
		}
	}
	return Py_BuildNone();
}
PyObject * playerOtoAvDurumAl(PyObject* poSelf, PyObject*poArgs){
	CInstanceBase * pInstance = CPythonCharacterManager::Instance().GetSelectedInstancePtr();
	if (!pInstance){ return Py_BuildValue("i", 0); }
	return Py_BuildValue("i", CPythonPlayer::Instance().OtoAvDurumAl());
}
#endif
...
void initPlayer()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetMainCharacterID",			playerGetMainCharacterID,			METH_VARARGS },
...
#ifdef ENABLE_RANK_SYSTEM
		{ "GetRank",					playerGetRankData,					METH_VARARGS },
#endif
#ifdef ENABLE_TEAM_SYSTEM
		{ "GetTeam",					playerGetTeamData,					METH_VARARGS },
#endif
#ifdef ENABLE_LANDRANK_SYSTEM
		{ "GetLandRank",				playerGetLandRankData,				METH_VARARGS },
		{ "GetLandRankString",			playerGetLandRankString,			METH_VARARGS },
#endif
#ifdef ENABLE_REBORN_SYSTEM
		{ "GetReborn",					playerGetRebornData,				METH_VARARGS },
#endif
#ifdef ENABLE_MONIKER_SYSTEM
		{ "GetMoniker",					playerGetMoniker,					METH_VARARGS },
#endif
#ifdef ENABLE_LOVE_SYSTEM
		{ "GetLove1",					playerGetLove1,						METH_VARARGS },
		{ "GetLove2",					playerGetLove2,						METH_VARARGS },
#endif // ENABLE_LOVE_SYSTEM
#ifdef ENABLE_WORD_SYSTEM
		{ "GetWord",					playerGetWord,						METH_VARARGS },
#endif // ENABLE_WORD_SYSTEM
		...
		{ "GetItemEvolutionType",		playerGetItemEvolutionType,			METH_VARARGS },
		{ "GetItemRealEvolution",		playerGetItemRealEvolution,			METH_VARARGS },
		{ "GetItemEvolutionAtk",		playerGetItemEvolutionAtk,			METH_VARARGS },
		{ "GetItemEvolutionElementAtk",	playerGetItemEvolutionElementAtk,	METH_VARARGS },
	#ifdef ENABLE_ARMOR_EVOLUTION_SYSTEM
		{ "GetItemEvolutionArmorElementAtk",	playerGetItemEvolutionArmorElementAtk,	METH_VARARGS },
		{ "GetItemEvolutionArmorAtk",	playerGetItemEvolutionArmorAtk,		METH_VARARGS },
	#endif
#endif
...
#endif
#ifdef ENABLE_UNBUG_CHARACTER_SYSTEM
		{ "UnBugCharacter",				playerUnBugCharacter,				METH_VARARGS },
#endif
		{ "PartyMemberVIDToPID",					playerPartyMemberVIDToPID,					METH_VARARGS },
#ifdef ENABLE_SCP1453_EXTENTIONS
		{"UseItemByVnum",	playerUseItemByVnum,	METH_VARARGS},
#endif
#ifdef ENABLE_CHEQUE_SYSTEM
		{"GetCheque", playerGetCheque, METH_VARARGS},
#endif
#ifdef ENABLE_AUTO_HUNT_SYSTEM
		{ "OtoAvSaldiriAta", playerOtoAvSaldiriAta, METH_VARARGS },
		{ "OtoAvSaldiriAl", playerOtoAvSaldiriAl, METH_VARARGS },
		{ "OtoAvOdakAta", playerOtoAvOdakAta, METH_VARARGS },
		{ "OtoAvOdakAl", playerOtoAvOdakAl, METH_VARARGS },
		{ "OtoAvDurumAta", playerOtoAvDurumAta, METH_VARARGS },
		{ "OtoAvDurumAl", playerOtoAvDurumAl, METH_VARARGS },
#endif
		{ NULL,							NULL,								NULL },
	};
	...
	PyModule_AddIntConstant(poModule, "SKILL_GRADE_EXPERT_MASTER", CPythonPlayer::SKILL_EXPERT_MASTER);
	PyModule_AddIntConstant(poModule, "SKILL_GRADE_DESTANSI_MASTER", CPythonPlayer::SKILL_DESTANSI_MASTER);
	PyModule_AddIntConstant(poModule, "SKILL_GRADE_TANRISAL_MASTER", CPythonPlayer::SKILL_TANRISAL_MASTER);
#endif
	...
	PyModule_AddIntConstant(poModule, "ACCESSORY_SOCKET_MAX_NUM", ITEM_ACCESSORY_SOCKET_MAX_NUM);
...
#ifdef ENABLE_PARTY_ROLE_REWORK
	PyModule_AddIntConstant(poModule, "PARTY_STATE_ATTACKER_MONSTER", CPythonPlayer::PARTY_ROLE_ATTACKER_MONSTER);
	PyModule_AddIntConstant(poModule, "PARTY_STATE_ATTACKER_STONE", CPythonPlayer::PARTY_ROLE_ATTACKER_STONE);
	PyModule_AddIntConstant(poModule, "PARTY_STATE_ATTACKER_BOSS", CPythonPlayer::PARTY_ROLE_ATTACKER_BOSS);
#endif
	...
	PyModule_AddIntConstant(poModule, "BLOCK_COSTUME_BODY", BLOCK_COSTUME_BODY);
	PyModule_AddIntConstant(poModule, "BLOCK_COSTUME_HAIR", BLOCK_COSTUME_HAIR);
	PyModule_AddIntConstant(poModule, "BLOCK_COSTUME_WEAPON", BLOCK_COSTUME_WEAPON);
	PyModule_AddIntConstant(poModule, "BLOCK_COSTUME_ACCE", BLOCK_COSTUME_ACCE);
	PyModule_AddIntConstant(poModule, "BLOCK_COSTUME_ACCE_EX", BLOCK_COSTUME_ACCE_EX);
	PyModule_AddIntConstant(poModule, "BLOCK_COSTUME_AURA", BLOCK_COSTUME_AURA);
	...
	PyModule_AddIntConstant(poModule, "SLOT_TYPE_FLOWER_INVENTORY", SLOT_TYPE_FLOWER_INVENTORY);
	...
	PyModule_AddIntConstant(poModule, "FLOWER_INVENTORY", FLOWER_INVENTORY);
...
#endif
#ifdef ENABLE_ITEM_EVOLUTION_SYSTEM
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_FIRE", WEAPON_EVOLUTION_TYPE_FIRE);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_ICE", WEAPON_EVOLUTION_TYPE_ICE);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_EARTH", WEAPON_EVOLUTION_TYPE_EARTH);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_WIND", WEAPON_EVOLUTION_TYPE_WIND);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_DARK", WEAPON_EVOLUTION_TYPE_DARK);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_ELEC", WEAPON_EVOLUTION_TYPE_ELEC);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_NONE", WEAPON_EVOLUTION_TYPE_NONE);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_ALL", WEAPON_EVOLUTION_TYPE_ALL);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_HUMAN", WEAPON_EVOLUTION_TYPE_HUMAN);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_EX_FIRE", WEAPON_EVOLUTION_TYPE_EX_FIRE);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_EX_ICE", WEAPON_EVOLUTION_TYPE_EX_ICE);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_EX_EARTH", WEAPON_EVOLUTION_TYPE_EX_EARTH);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_EX_WIND", WEAPON_EVOLUTION_TYPE_EX_WIND);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_EX_DARK", WEAPON_EVOLUTION_TYPE_EX_DARK);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_EX_ELEC", WEAPON_EVOLUTION_TYPE_EX_ELEC);
	PyModule_AddIntConstant(poModule, "WEAPON_EVOLUTION_TYPE_EX_ALL", WEAPON_EVOLUTION_TYPE_EX_ALL);
	PyModule_AddIntConstant(poModule, "EV_CLEAN_VALUE0", EV_CLEAN_VALUE0);
#endif
#ifdef ENABLE_CHEQUE_SYSTEM
	...
	PyModule_AddIntConstant(poModule, "SPECIAL_WINDOW_OFFLINE_SHOP", SPECIAL_WINDOW_OFFLINE_SHOP);
#endif
}