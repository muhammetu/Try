...
PyObject* chatStopChatToggle(PyObject* poSelf, PyObject* poArgs)
{
	CPythonChat::Instance().StopChatToggle();
	return Py_BuildNone();
}
PyObject* chatIsStoppedChat(PyObject* poSelf, PyObject* poArgs)
{
	return Py_BuildValue("i", CPythonChat::Instance().IsStoppedChat());
}
#endif
...
PyObject* chatGetLinkFromHyperlink(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (0 == results[0].compare("item"))
	{
		...
		if (CItemManager::Instance().GetItemDataPointer(htoi(results[1].c_str()), &pItemData))
		{
...
#ifdef ENABLE_ITEM_ENTITY_UTILITY
			len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%I64d", atoi(results[8].c_str()));
#endif

#ifdef ENABLE_CHANGELOOK_SYSTEM
			len += snprintf(itemlink + len, sizeof(itemlink) - len, ":%I64d", atoi(results[9].c_str()));
#endif

#ifdef ENABLE_ITEM_EVOLUTION_SYSTEM
			if (results.size() >= 12)
			{
				for (int i = 8; i < results.size(); i += 2)
#else
			if (results.size() >= 11)
			{
			...
			if (atoi(results[8].c_str()) > 1)
				if (isAttr)
					snprintf(buf, sizeof(buf), "|cffffc700|H%s|h[%s x%I64d]|h|r", itemlink, szText, atoi(results[8].c_str()));
				else
					snprintf(buf, sizeof(buf), "|cfff1e6c0|H%s|h[%s x%I64d]|h|r", itemlink, szText, atoi(results[8].c_str()));
			else
				if (isAttr)
					snprintf(buf, sizeof(buf), "|cffffc700|H%s|h[%s]|h|r", itemlink, szText);
				else
...
PyObject* chatGetChatStack(PyObject * poSelf, PyObject * poArgs)
{
	...
	std::string stack;
	CPythonChat::Instance().GetChatStack(index, stack);
	printf("chatGetChatStack: %s\n", stack.c_str());
	return Py_BuildValue("s", stack.c_str());
}
...
void initChat()
{
...
#endif
#ifdef ENABLE_CHAT_STOP
		{ "StopChatToggle",			chatStopChatToggle,		METH_VARARGS },
		{ "IsStoppedChat",			chatIsStoppedChat,		METH_VARARGS },
#endif
#ifdef ENABLE_CHAT_STACK_SYSTEM
	...
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_NOTICE_IMPROVING", CHAT_TYPE_NOTICE_IMPROVING);
#endif
#ifdef ENABLE_NEW_DUNGEON_TYPE
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_DUNGEON_MISSION", CHAT_TYPE_DUNGEON_MISSION);
	PyModule_AddIntConstant(poModule, "CHAT_TYPE_DUNGEON_SUBMISSION", CHAT_TYPE_DUNGEON_SUBMISSION);
#endif
	...
	PyModule_AddIntConstant(poModule, "CHAT_FILTER_PARTY",	CHAT_FILTER_PARTY);
	PyModule_AddIntConstant(poModule, "CHAT_FILTER_GUILD",	CHAT_FILTER_GUILD);
	PyModule_AddIntConstant(poModule, "CHAT_FILTER_SHOUT",	CHAT_FILTER_SHOUT);
	PyModule_AddIntConstant(poModule, "CHAT_FILTER_MAX_NUM", CHAT_FILTER_MAX_NUM);
#else
	PyModule_AddIntConstant(poModule, "CHAT_FILTER_MAX_NUM", 3);
#endif
}