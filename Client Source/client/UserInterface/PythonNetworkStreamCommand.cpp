...
bool CPythonNetworkStream::ClientCommand(const char* c_szChat)
{
	return false;
}
...
void CPythonNetworkStream::ServerCommand(char* c_szCommand)
{
	...
	else if (!strcmpi(szCmd, "cube"))
	{
		...
		else if ("info" == TokenVector[1])
		{
			...
			long long gold = atoi(TokenVector[2].c_str());
			UINT itemVnum = atoi(TokenVector[3].c_str());
			UINT count = atoi(TokenVector[4].c_str());
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_Cube_UpdateInfo", Py_BuildValue("(Lii)", gold, itemVnum, count));
#else
...
#endif
#ifdef ENABLE_BAN_REASON_UTILITY
	else if (!strcmpi(szCmd, "ExitApplication"))
	{
