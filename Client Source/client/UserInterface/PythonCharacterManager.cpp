...
#ifdef USE_DISCORD_RPC_MODULE
	CDiscordManager::Instance().UpdatePresence(CDiscordManager::EDiscordPresenceStatus::Discord_Status_PVP, dwVIDSrc);
#endif
}
...
#ifdef USE_DISCORD_RPC_MODULE
	CDiscordManager::Instance().UpdatePresence(CDiscordManager::EDiscordPresenceStatus::Discord_Status_PVM, dwVIDSrc);
#endif
}
...
CInstanceBase* CPythonCharacterManager::CreateInstance(const CInstanceBase::SCreateData& c_rkCreateData, bool bCreateCharacter)
#else
...
CInstanceBase * CPythonCharacterManager::OtomatikHedefBul(CInstanceBase * pkInstMain, bool lock)
{
	if (!pkInstMain)
		return nullptr;
	...
	if (!lock && rkPlayer.GetTargetVID() != 0)
	{
		CInstanceBase* pInstanceTarget = CPythonCharacterManager::Instance().GetInstancePtr(rkPlayer.GetTargetVID());
		if (pInstanceTarget)
	...
	for (TCharacterInstanceMap::iterator itor = m_kAliveInstMap.begin(); itor != m_kAliveInstMap.end(); ++itor)
	{
		...
		if (!iTarget)
			continue;
		if (iTarget->HasAttacker() != 0)
		{
			if (iTarget->HasAttacker() != pkInstMain->GetVirtualID())
				continue;
		}
		if (iTarget->GetAttacked() == 1)
			continue;
		...
		{
			CInstanceBase* pInstanceTarget = CPythonCharacterManager::Instance().GetInstancePtr(rkPlayer.GetTargetVID());
			if (pInstanceTarget && rkPlayer.GetTargetVID() == iTarget->GetVirtualID())
				continue;
		}
...
void CPythonCharacterManager::RefreshAllStone()
{
	return;
	float stoneLevel = 1.0f + static_cast<float>(CPythonSystem::Instance().GetStoneSize() / 10.0f);

	TCharacterInstanceMap::iterator itor = m_kAliveInstMap.begin();
	for (; itor != m_kAliveInstMap.end(); ++itor)
	{
		CInstanceBase* pInstance = itor->second;
		if (!pInstance->IsStone())
			continue;

		pInstance->GetGraphicThingInstancePtr()->ClearAttachingEffect();
		pInstance->DestroyContainer();

		// set scale
		pInstance->GetGraphicThingInstancePtr()->SetScaleWorld(stoneLevel, stoneLevel, stoneLevel);

		// set container
		pInstance->AttachContainer();
...
void CPythonCharacterManager::RefreshAllMobs()
{
	float mobLevel = 1.0f - static_cast<float>(CPythonSystem::Instance().GetMobSize()/14.0f);

	TCharacterInstanceMap::iterator itor = m_kAliveInstMap.begin();
	for (; itor != m_kAliveInstMap.end(); ++itor)
	{
		CInstanceBase* pInstance = itor->second;
		if (pInstance->IsBoss() || pInstance->IsPC() || pInstance->IsStone() || pInstance->IsPet() || pInstance->IsNPC() || pInstance->IsNewPet() || pInstance->IsNewExMount())
			continue;

		pInstance->GetGraphicThingInstancePtr()->ClearAttachingEffect();
		pInstance->GetGraphicThingInstancePtr()->SetScaleWorld(mobLevel, mobLevel, mobLevel);
		pInstance->GetGraphicThingInstancePtr()->RefreshActorInstance();
		pInstance->Refresh(CRaceMotionData::NAME_WAIT, false);
		pInstance->DetachTextTail();
		pInstance->AttachTextTail();
...
void CPythonCharacterManager::RefreshAllNewPets()
{
	float stoneLevel = 1.0f + static_cast<float>(CPythonSystem::Instance().GetPetSize() / 5.0f);

	TCharacterInstanceMap::iterator itor = m_kAliveInstMap.begin();
	for (; itor != m_kAliveInstMap.end(); ++itor)
	{
		CInstanceBase* pInstance = itor->second;
		if (!pInstance->IsNewPet() && !pInstance->IsNewExMount())
			continue;
