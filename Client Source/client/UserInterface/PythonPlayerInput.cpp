...
void CPythonPlayer::__SetTargetVID(DWORD dwVID)
{
	...
	tokucbaboex(ATTACK, ptrti, dwVID);
#endif
	m_dwTargetVID = dwVID;
}
...
void CPythonPlayer::__OnPressActor(CInstanceBase& rkInstMain, DWORD dwPickedActorID, bool isAuto)
{
	...
	// @duzenleme oto ataklarla alakali martySama adli arkadasin yaptigi fix.
	if (isAuto)
	{
		if (rkInstMain.IsAttackableInstance(rkInstVictim))
...
void CPythonPlayer::__ReserveProcess_ClickActor()
{
	...
	if (pkInstReserved->GetAttacked() == 1 && OtoAvSaldiriAl() && OtoAvOdakAl())
	{
		__ClearReservedAction();
		return;
	}
...
#ifdef ENABLE_AUTO_HUNT_SYSTEM
	if (pkInstReserved->GetAttacked() == 0)
		pkInstReserved->SetAttacked(1);
#endif // ENABLE_AUTO_HUNT_SYSTEM
