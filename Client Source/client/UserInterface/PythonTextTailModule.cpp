...
PyObject * textTailIsCharacter(PyObject * poSelf, PyObject * poArgs)
{
	int VirtualID;
	if (!PyTuple_GetInteger(poArgs, 0, &VirtualID))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonTextTail::Instance().IsCharacterTextTail(VirtualID));
}

PyObject * textTailIsItem(PyObject * poSelf, PyObject * poArgs)
{
	int VirtualID;
	if (!PyTuple_GetInteger(poArgs, 0, &VirtualID))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonTextTail::Instance().IsItemTextTail(VirtualID));
}

PyObject* textTailGetInstanceType(PyObject * poSelf, PyObject * poArgs)
{
	int VirtualID;
	if (!PyTuple_GetInteger(poArgs, 0, &VirtualID))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonTextTail::Instance().GetInstanceType(VirtualID));
}
#endif
...
#ifdef ENABLE_GRAPHIC_OPTIMIZATION
		{ "IsCharacter",				textTailIsCharacter,				METH_VARARGS },
		{ "IsItem",						textTailIsItem,						METH_VARARGS },
		{ "GetInstanceType",			textTailGetInstanceType,			METH_VARARGS },
#endif
		{ NULL, NULL, NULL },
	};
