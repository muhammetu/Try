...
std::map<DWORD, std::string> affectNameMap = {
	// { CInstanceBase::AFFECT_BIO_BONUS1, "Biyolog I"},
	// { CInstanceBase::AFFECT_BIO_BONUS2, "Biyolog II"},
	// { CInstanceBase::AFFECT_BIO_BONUS3, "Biyolog II"},
	// { CInstanceBase::AFFECT_BIO_BONUS4, "Biyolog IV"},
	// { CInstanceBase::AFFECT_BIO_BONUS5, "Biyolog V"},
	// { CInstanceBase::NEW_AFFECT_EXP_BONUS, "EXP"},
};
...
PyObject* effectGetName(PyObject* poSelf, PyObject* poArgs)
{
	int iType;
	if (!PyTuple_GetInteger(poArgs, 0, &iType))
		return Py_BuildException();

	auto it = affectNameMap.find(iType);

	return Py_BuildValue("s", it == affectNameMap.end() ? "" : it->second.c_str());
}

PyObject* effectSetName(PyObject* poSelf, PyObject* poArgs)
{
	int iType;
	if (!PyTuple_GetInteger(poArgs, 0, &iType))
		return Py_BuildException();
	...
	affectNameMap.insert({iType, szName});
...
void initeffect()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetName",					effectGetName,					METH_VARARGS },
		{ "SetName",					effectSetName,					METH_VARARGS },
	...
	PyModule_AddIntConstant(poModule, "FLY_NONE", CFlyingManager::FLY_NONE);
	PyModule_AddIntConstant(poModule, "FLY_EXP", CFlyingManager::FLY_EXP);
	PyModule_AddIntConstant(poModule, "FLY_HP_MEDIUM", CFlyingManager::FLY_HP_MEDIUM);
	PyModule_AddIntConstant(poModule, "FLY_HP_BIG", CFlyingManager::FLY_HP_BIG);
	PyModule_AddIntConstant(poModule, "FLY_SP_SMALL", CFlyingManager::FLY_SP_SMALL);
	PyModule_AddIntConstant(poModule, "FLY_SP_MEDIUM", CFlyingManager::FLY_SP_MEDIUM);
	PyModule_AddIntConstant(poModule, "FLY_SP_BIG", CFlyingManager::FLY_SP_BIG);
	PyModule_AddIntConstant(poModule, "FLY_FIREWORK1", CFlyingManager::FLY_FIREWORK1);
	PyModule_AddIntConstant(poModule, "FLY_FIREWORK2", CFlyingManager::FLY_FIREWORK2);
	PyModule_AddIntConstant(poModule, "FLY_FIREWORK3", CFlyingManager::FLY_FIREWORK3);
	PyModule_AddIntConstant(poModule, "FLY_FIREWORK4", CFlyingManager::FLY_FIREWORK4);
	PyModule_AddIntConstant(poModule, "FLY_FIREWORK5", CFlyingManager::FLY_FIREWORK5);
	PyModule_AddIntConstant(poModule, "FLY_FIREWORK6", CFlyingManager::FLY_FIREWORK6);
	PyModule_AddIntConstant(poModule, "FLY_FIREWORK_XMAS", CFlyingManager::FLY_FIREWORK_XMAS);
	PyModule_AddIntConstant(poModule, "FLY_CHAIN_LIGHTNING", CFlyingManager::FLY_CHAIN_LIGHTNING);
	PyModule_AddIntConstant(poModule, "FLY_HP_SMALL", CFlyingManager::FLY_HP_SMALL);
	PyModule_AddIntConstant(poModule, "FLY_SKILL_MUYEONG", CFlyingManager::FLY_SKILL_MUYEONG);
#ifdef ENABLE_QUIVER_SYSTEM
	PyModule_AddIntConstant(poModule, "FLY_QUIVER_ATTACK_NORMAL", CFlyingManager::FLY_QUIVER_ATTACK_NORMAL);
#endif
