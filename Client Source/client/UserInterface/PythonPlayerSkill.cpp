...
bool CPythonPlayer::__CheckShortMana(TSkillInstance& rkSkillInst, CPythonSkill::TSkillData& rkSkillData)
{
	...
	if (!rkSkillData.IsToggleSkill())
	{
		if (iNeedSP == 0)
		{
			CPythonChat::Instance().AppendChat(CHAT_TYPE_INFO, "Oyun dosyalarý hatalý. Lütfen Vatansever'yi baþtan indirin.");
			return true;
		}
	}
...
bool CPythonPlayer::__HasItem(DWORD dwItemID)
{
	...
	for (int j = 0; j < c_Special_ItemSlot_Count; ++j)
	{
		...
		if (dwItemID == GetItemIndex(TItemPos(STONE_INVENTORY, j)))
			return true;
		if (dwItemID == GetItemIndex(TItemPos(FLOWER_INVENTORY, j)))
			return true;
		if (dwItemID == GetItemIndex(TItemPos(ATTR_INVENTORY, j)))
			return true;
