...
int CPythonSwitchbot::GetAttibuteSet(BYTE slot)
{
	...
	BYTE item_type = 0, item_subtype = 0;
...
bool CPythonSwitchbot::IsActiveWindow()
{
...
PyObject* switchbotChangeSpeed(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bSpeed;
	if (!PyTuple_GetByte(poArgs, 0, &bSpeed))
		return Py_BuildException();

	if (bSpeed > 3)
		return Py_BuildNone();

	CPythonNetworkStream::Instance().SendSwitchbotChangeSpeed(bSpeed);
	return Py_BuildNone();
}
...
void initSwitchbot()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "ChangeSpeed",			switchbotChangeSpeed,			METH_VARARGS },
