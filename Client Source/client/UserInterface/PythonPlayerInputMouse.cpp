...
void CPythonPlayer::NEW_RefreshMouseWalkingDirection()
{
	...
	case MODE_CLICK_ITEM:
	{
		...
		{
			// Burada yerdeki iteme veya örneðin metne týklayýnca bineðin onun etrafýnda dönmesi giderilmiþ oldu.
	...
	if (m_isAtkKey && CPythonApplication::Instance().IsActivateWnd() && !CPythonApplication::Instance().IsMinimalizeWnd())
		NEW_Attack();
