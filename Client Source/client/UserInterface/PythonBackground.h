...
public:
	void LoadMapIndex();
	int GetMapIndex(std::string stMapName);
	std::string GetMapName(int mapIndex);
	std::string GetRealMapName(std::string stMapName);

protected:
	struct TMapIndexInfo
	{
		std::string stMapName;
		int iMapIdx;
		std::string stRealName;
	};
	std::vector<TMapIndexInfo>	m_vecMapIndex;
#endif
};
