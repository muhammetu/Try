...
PyObject* miniMapIsCheckedAtlasMarker(PyObject * poSelf, PyObject * poArgs)
{
	int type;
	if (!PyTuple_GetInteger (poArgs, 0, &type))
		return Py_BuildException();

	bool returnValue = CPythonMiniMap::Instance().IsCheckedAtlasMarker (type);
	return Py_BuildValue ("i", (int)returnValue);
}
#endif
...
void initMiniMap()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "IsCheckedAtlasMarker",			miniMapIsCheckedAtlasMarker,					METH_VARARGS },
#endif
	...
	PyModule_AddIntConstant(poModule, "TYPE_STONE",		CPythonMiniMap::TYPE_STONE);
	PyModule_AddIntConstant(poModule, "TYPE_BOSS",		CPythonMiniMap::TYPE_BOSS);

#ifdef ENABLE_ATLAS_RENDER_OPTION
	PyModule_AddIntConstant(poModule, "MARKER_MAX",	CPythonMiniMap::MARKER_MAX);
#endif
}