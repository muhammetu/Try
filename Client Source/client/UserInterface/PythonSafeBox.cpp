...
PyObject* safeboxGetItemEvolution(PyObject* poSelf, PyObject* poArgs)
{
	int ipos;
	if (!PyTuple_GetInteger(poArgs, 0, &ipos))
		return Py_BadArgument();
...
PyObject* safeboxGetMallItemEvolution(PyObject* poSelf, PyObject* poArgs)
{
	int ipos;
	if (!PyTuple_GetInteger(poArgs, 0, &ipos))
		return Py_BadArgument();
...
{
#ifdef ENABLE_GOLD_LIMIT_REWORK
	return Py_BuildValue("L", CPythonSafeBox::Instance().GetMoney());
#else
...
#ifdef ENABLE_ITEM_EVOLUTION_SYSTEM
		{ "GetItemEvolution",			safeboxGetItemEvolution,				METH_VARARGS },
		{ "GetMallItemEvolution",		safeboxGetMallItemEvolution,			METH_VARARGS },
#endif
#ifdef ENABLE_CHANGELOOK_SYSTEM
