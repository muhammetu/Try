...
{
#ifdef ENABLE_GRAPHIC_OPTIMIZATION
	BYTE dropItemLevel = CPythonSystem::Instance().GetDropItemLevel();
#endif
	TGroundItemInstanceMap::iterator itor = m_GroundItemInstanceMap.begin();
	for (; itor != m_GroundItemInstanceMap.end(); ++itor)
	{
#ifdef ENABLE_GRAPHIC_OPTIMIZATION
		// @ efektleri buradan gizleyecegiz.
		CEffectManager::Instance().SelectEffectInstance(itor->second->dwEffectInstanceIndex);
		if (dropItemLevel >= 1)
...
#ifdef ENABLE_ITEM_ENTITY_UTILITY
void CPythonItem::CreateItem(DWORD dwVirtualID, DWORD dwVirtualNumber, float x, float y, float z, bool bDrop, long alSockets[ITEM_SOCKET_SLOT_MAX_NUM], DWORD wCount)
#else
...
#endif
{
	...
	if (bDrop)
	{
		...
		if ((pItemData->GetType() == CItemData::ITEM_TYPE_COSTUME && pItemData->GetSubType() == CItemData::COSTUME_WEAPON) && (pItemData->GetValue(3) == CItemData::WEAPON_SWORD || pItemData->GetValue(3) == CItemData::WEAPON_ARROW))
			bStabGround = true;
#endif
#ifdef ENABLE_QUIVER_SYSTEM
	...
	if (wCount > 1)
		len += snprintf(szItemName + len, sizeof(szItemName) - len, (len > 0) ? " %s (x%d)" : "%s (x%d)", pItemData->GetName(), wCount);
	else
...
#ifdef ENABLE_PICKUP_ITEM_REWORK
std::vector<DWORD> CPythonItem::GetCloseItemVector(const std::string& myName, const TPixelPosition& c_rPixelPosition) const
{
	...
	static const float fCloseItemDistance = std::powf(1500.0f, 2);
