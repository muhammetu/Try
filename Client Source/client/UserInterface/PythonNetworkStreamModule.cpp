...
PyObject* netSetLoginInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!PyTuple_GetString(poArgs, 2, &szClientVersion))
		return Py_BuildException();
#endif
...
PyObject* netSendCreateCharacterPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendCreateCharacterPacket((BYTE)index, name, (BYTE)job, (BYTE)shape);
	return Py_BuildNone();
}
...
PyObject* netSendItemUsePacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
#ifdef ENABLE_USE_ITEM_COUNT
	rkNetStream.SendItemUsePacket(Cell, count);
...
PyObject* netSendItemMovePacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	rkNetStream.SendItemMovePacket(Cell, ChangeCell, (DWORD)num);
#else
...
PyObject* netSendGuildRemoveMemberPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	{
		TraceError(XOR("netSendGuildRemoveMemberPacket(szKey=%s) - Can't Find Guild Member\n"), szKey);
		return Py_BuildNone();
	}
...
PyObject* netSendChestDropInfo(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendChestDropInfo(vnum, pos);
...
PyObject* netDungeonTeleport(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.DungeonTeleport(index);
	return Py_BuildNone();
}
#endif

#ifdef ENABLE_CHANNEL_INFO_UPDATE
PyObject* netGetChannel(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& net = CPythonNetworkStream::Instance();
	return Py_BuildValue("i", net.GetChannel());
}
#endif
...
{
	DWORD wValue = 0;
	if (!PyTuple_GetUnsignedLong(poArgs, 0, &wValue))
		return Py_BuildException();
...
PyObject* netSendMobInfoPacket(PyObject* poSelf, PyObject* poArgs)
{
	DWORD dwMobVnum = 0;
	if (!PyTuple_GetUnsignedLong(poArgs, 0, &dwMobVnum))
		return Py_BuildException();

	DWORD dwVirtualID = 0;
	if (!PyTuple_GetUnsignedLong(poArgs, 1, &dwVirtualID))
		return Py_BuildException();

	BYTE bType = 0;
	if (!PyTuple_GetByte(poArgs, 2, &bType))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendMobInfoPacket(dwMobVnum, dwVirtualID, bType);
	return Py_BuildNone();
}
#endif
...
#ifdef USE_LOADING_DLG_OPTIMIZATION
PyObject* netInitLoadThread(PyObject* poSelf, PyObject* poArgs)
{
	CPlayerSettingsModule::Instance().startLoadThread();
	return Py_BuildNone();
}
#endif // USE_LOADING_DLG_OPTIMIZATION
...
PyObject* netSendBattlePassAction(PyObject* poSelf, PyObject* poArgs)
{
	...
	int iSubAction = 0;
	if (!PyTuple_GetInteger(poArgs, 1, &iSubAction))
		return Py_BuildException();

	CPythonNetworkStream::Instance().SendBattlePassAction(iAction, iSubAction);
	return Py_BuildNone();
}
#endif

#ifdef ENABLE_AUTO_HUNT_SYSTEM
PyObject* netSetCikiyorumStatus(PyObject* poSelf, PyObject* poArgs)
{
	int iAction = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &iAction))
		return Py_BuildException();
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	rns.SetCikiyorumStatus(iAction);
	return Py_BuildNone();
}

PyObject* isCikiyorumStatus(PyObject* poSelf, PyObject* poArgs)
{
	CPythonNetworkStream& rns = CPythonNetworkStream::Instance();
	return Py_BuildValue("i", rns.IsCikiyorumStatus());
}
#endif
...
#endif
#ifdef ENABLE_CHANNEL_INFO_UPDATE
		{ "GetChannel",								netGetChannel,								METH_VARARGS },
#endif
#ifdef ENABLE_CHEQUE_SYSTEM
...
#endif
#ifdef ENABLE_TARGET_BOARD_RENEWAL
		{ "SendMobInfoPacket",								netSendMobInfoPacket,						METH_VARARGS},
#endif
#ifdef USE_LOADING_DLG_OPTIMIZATION
		{ "InitLoadThread",											netInitLoadThread,									METH_VARARGS },
#endif // USE_LOADING_DLG_OPTIMIZATION
#ifdef ENABLE_BATTLE_PASS_SYSTEM
		{ "SendBattlePassAction", netSendBattlePassAction, METH_VARARGS },
#endif
#ifdef ENABLE_AUTO_HUNT_SYSTEM
		{ "SetCikiyorumStatus",	netSetCikiyorumStatus,	METH_VARARGS },
		{ "isCikiyorumStatus", isCikiyorumStatus,	METH_VARARGS },
#endif
		{ NULL,										NULL,										NULL },
	};
...
#ifdef ENABLE_TARGET_BOARD_RENEWAL
	PyModule_AddIntConstant(poModule, "MOB_INFORMATION_DROP", MOB_INFORMATION_DROP);
	PyModule_AddIntConstant(poModule, "MOB_INFORMATION_BONUS", MOB_INFORMATION_BONUS);
	PyModule_AddIntConstant(poModule, "MOB_INFORMATION_SPECIALITY", MOB_INFORMATION_SPECIALITY);
	PyModule_AddIntConstant(poModule, "MOB_INFORMATION_DUNGEON_DROP", MOB_INFORMATION_DUNGEON_DROP);
#endif
}