	...
	inline bool InFrustum(CParticleInstance * pInstance)
	{
		if (m_pParticleProperty->m_bAttachFlag)
			return CScreen::GetFrustum().ViewVolumeTest(Vector3d(
				...
				, Frustum::FRUSTUM_TYPE_EFFECT
#endif
				)!=VS_OUTSIDE;
		else
#ifdef ENABLE_GRAPHIC_OPTIMIZATION
			return CScreen::GetFrustum().ViewVolumeTest(Vector3d(pInstance->m_v3Position.x,pInstance->m_v3Position.y,pInstance->m_v3Position.z),pInstance->GetRadiusApproximation(), Frustum::FRUSTUM_TYPE_EFFECT)!=VS_OUTSIDE;
#else
