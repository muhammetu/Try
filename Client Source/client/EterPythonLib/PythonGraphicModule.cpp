...
PyObject* grpCreateTextBarNew(PyObject* poSelf, PyObject* poArgs)
{
	int iWidth;
	if (!PyTuple_GetInteger(poArgs, 0, &iWidth))
		return Py_BuildException();
	...
	if (!PyTuple_GetInteger(poArgs, 2, &iFontSize))
		return Py_BuildException();
	int iIsBold;
	if (!PyTuple_GetInteger(poArgs, 3, &iIsBold))
		return Py_BuildException();

	CTextBar* pTextBar = new CTextBar(iFontSize, iIsBold);
	if (!pTextBar->Create(NULL, iWidth, iHeight))
	{
		delete pTextBar;
		return Py_BuildValue("i", NULL);
...
PyObject* grpSetTextBarColor(PyObject* poSelf, PyObject* poArgs)
{
	int iHandle;
	if (!PyTuple_GetInteger(poArgs, 0, &iHandle))
		return Py_BuildException();
	...
	CTextBar* pTextBar = (CTextBar*)iHandle;
	if (pTextBar)
		pTextBar->SetColor(D3DXCOLOR(dwColor));
...
void initgrp()
{
	static PyMethodDef s_methods[] =
	{
...
#ifdef ENABLE_WIKI_SYSTEM
		{ "CreateTextBarNew",			grpCreateTextBarNew,			METH_VARARGS },
#endif
		...
		{ "SetTextBarColor",			grpSetTextBarColor,				METH_VARARGS },
