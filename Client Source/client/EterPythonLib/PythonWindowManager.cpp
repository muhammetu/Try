...
namespace UI
{
	...
	float CWindowManager::PrepareClipRect(bool getRight, CWindow* pFirstWindow, CWindow* pSecondWindow, bool isSlot, DWORD slotWidth, DWORD slotHeight, DWORD slotYParameter)
	{
		const RECT& pFirstWindowRect = pFirstWindow->GetRect();
		const RECT& pSecondWindowRect = pSecondWindow->GetRect();

		float firstWindowRight = static_cast<float>(pFirstWindowRect.left + pFirstWindow->GetWidth());
		float firstWindowBottom = static_cast<float>(pFirstWindowRect.top + pFirstWindow->GetHeight() + slotYParameter);

		float secondWindowRight = static_cast<float>(pSecondWindowRect.left + pSecondWindow->GetWidth());
		float secondWindowBottom = static_cast<float>(pSecondWindowRect.top + pSecondWindow->GetHeight());

		float fDifRight = 0.0f;
		float fDifBottom = 0.0f;

		if (isSlot)
		{
			fDifRight = 0.0f;
			firstWindowBottom = static_cast<float>(pFirstWindowRect.top + slotYParameter);
			fDifBottom = (firstWindowBottom > secondWindowBottom) ? -float((firstWindowBottom - secondWindowBottom) / float(slotHeight)) : 0.0f;
		}
		else
		{
			fDifRight = (firstWindowRight > secondWindowRight) ? -float((firstWindowRight - secondWindowRight) / float(pFirstWindow->GetWidth())) : 0.0f;
			fDifBottom = (firstWindowBottom > secondWindowBottom) ? -float((firstWindowBottom - secondWindowBottom) / float(pFirstWindow->GetHeight())) : 0.0f;
		}
		...
		if (m_pLeftCaptureWindow)
		{
			...
			if (pWin->IsFlag(CWindow::FLAG_MOVABLE))
			{
				...
				if (pWin->IsFlag(CWindow::FLAG_LIMIT))
				{
					...
					x = max(x, limitRect.left);
					y = max(y, limitRect.top);
					x = min(x, max(0, limitRect.right - pWin->GetWidth()));
					y = min(y, max(0, limitRect.bottom - pWin->GetHeight()));
				}
