...
PyObject* grpGetImagePosition(PyObject* poSelf, PyObject* poArgs)
{
	CGraphicImageInstance* pImageInstance;
	if (!PyTuple_GetImageInstance(poArgs, 0, &pImageInstance))
		return Py_BadArgument();

	auto pos = pImageInstance->GetPosition();
...
PyObject* grpSetClipRect(PyObject* poSelf, PyObject* poArgs)
{
	CGraphicImageInstance* pImageInstance;
	if (!PyTuple_GetImageInstance(poArgs, 0, &pImageInstance))
		return Py_BadArgument();
	...
	float isx;
	if (!PyTuple_GetFloat(poArgs, 1, &isx))
		return Py_BuildException();
	float isy;
	if (!PyTuple_GetFloat(poArgs, 2, &isy))
		return Py_BuildException();
	float iex;
	if (!PyTuple_GetFloat(poArgs, 3, &iex))
		return Py_BuildException();
	float iey;
	if (!PyTuple_GetFloat(poArgs, 4, &iey))
		return Py_BuildException();

	pImageInstance->SetClipRect(isx, isy, iex, iey);
	return Py_BuildNone();
}
...
void initgrpImage()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetPosition",			grpGetImagePosition,			METH_VARARGS },
		...
		{ "SetClipRect",			grpSetClipRect,					METH_VARARGS },
