...
namespace UI
{
	...
	BOOL CWindow::RunMouseWheelEvent(long nLen)
	{
		...
		if (PyCallClassMemberFunc(m_poHandler, "OnRunMouseWheel", Py_BuildValue("(l)", nLen), &bValue))
			if (bValue == false)
				return GetParent()->RunMouseWheelEvent(nLen);
	...
	void CNumberLine::SetNumber(const char* c_szNumber)
	{
		...
		for (DWORD i = 0; i < m_strNumber.size(); ++i)
		{
			...
			{
				strImageFileName = m_strPath + "e.sub";
			}
			else if ('d' == cChar)
			{
				strImageFileName = m_strPath + "d.sub";
			}
#endif
	...
	BOOL CImageBox::LoadImage(const char* c_szFileName)
	{
		...
		if (!pResource)
		{
			TraceError("Resource not found: %s", c_szFileName);
			return FALSE;
		}
		if (!pResource->IsType(CGraphicImage::Type()))
		{
			TraceError("Resource is not a image: %s", c_szFileName);
			return FALSE;
		}
		...
		{
			TraceError("Resource is not a empty image: %s", c_szFileName);
			return FALSE;
		}
	...
	void CAniImageBox::SetDiffuseColorByType(BYTE byColorType)
	{
		...
		case COLOR_TYPE_PINK:
			...
			break;
		case COLOR_TYPE_DARK:
			fr = 0.38f; fg = 0.50f; fb = 0.56f; fa = 1.0f;
			break;
		default:
			return;
			break;
