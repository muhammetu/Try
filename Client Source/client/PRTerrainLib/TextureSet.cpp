	...
	float fTerrainTexCoordBase)
{
	...
	if (!pResource)// @duzenleme resource yoksa crash yemesin diye eklenmis.
		return false;
	...
	float fTerrainTexCoordBase)
{
	...
	CResource* pResource = CResourceManager::Instance().GetResourcePointer(c_szFileName);
	if (!pResource)// @burada koraydan bir kontrol alyndy crash amaçly
		return false;
	...
	// @duzenleme martySama arkadas boyle bir fixi uygun gormus.
	AddEmptyTexture();
