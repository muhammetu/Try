...
void CGraphicRenderTargetTexture::Render() const
{
	...
	const auto su = 0.0f;
	const auto sv = 0.0f;
	float eu = 1.0f / desc.Width * (m_renderRect.right - m_renderRect.left);
	float ev = 1.0f / desc.Height * (m_renderRect.bottom - m_renderRect.top);
	...
	pVertices[0].diffuse = diffuseColor;

	pVertices[1].position = TPosition(m_renderRect.left + m_renderRect.right - m_renderRect.left - 0.5f, m_renderRect.top - 0.5f, 0.0f);
	pVertices[1].texCoord = TTextureCoordinate(eu, sv);
	pVertices[1].diffuse = diffuseColor;

	pVertices[2].position = TPosition(m_renderRect.left - 0.5f, m_renderRect.top + m_renderRect.bottom - m_renderRect.top - 0.5f, 0.0f);
	pVertices[2].texCoord = TTextureCoordinate(su, ev);
	pVertices[2].diffuse = diffuseColor;

	pVertices[3].position = TPosition(m_renderRect.left + m_renderRect.right - m_renderRect.left - 0.5f, m_renderRect.top + m_renderRect.bottom - m_renderRect.top - 0.5f, 0.0f);
	pVertices[3].texCoord = TTextureCoordinate(eu, ev);
	pVertices[3].diffuse = diffuseColor;

	bool success = CGraphicBase::SetPDTStream(pVertices, 4);
	if(!success) 
	{
		TraceError("CGraphicRenderTargetTexture Render : Changing Vertices failed!");
		return;
	}
