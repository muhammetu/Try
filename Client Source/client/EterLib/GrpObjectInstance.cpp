...
BYTE CGraphicObjectInstance::PrepareFrustumType()
{
	if (GetType() == EFFECT_OBJECT)
#ifdef ENABLE_GRAPHIC_OPTIMIZATION
		return Frustum::FRUSTUM_TYPE_EFFECT;
#else
		return Frustum::FRUSTUM_TYPE_NORMAL;
#endif
	else if (GetType() == TREE_OBJECT)
#ifdef ENABLE_GRAPHIC_OPTIMIZATION
		return Frustum::FRUSTUM_TYPE_TREE;
#else
		return Frustum::FRUSTUM_TYPE_NORMAL;
#endif
...
#ifdef ENABLE_GRAPHIC_OPTIMIZATION
	m_CullingHandle->SetFrustumType(PrepareFrustumType());
#endif
}
