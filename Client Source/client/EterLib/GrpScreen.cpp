...
void CScreen::Show(HWND hWnd)
{
	...
	if (g_isBrowserMode)
	{
		RECT rcTop = { 0, 0, static_cast<LONG>(ms_d3dPresentParameter.BackBufferWidth), g_rcBrowser.top };
...
void CScreen::BuildViewFrustumAll()
{
	BuildViewFrustum(Frustum::FRUSTUM_TYPE_NORMAL);
#ifdef ENABLE_GRAPHIC_OPTIMIZATION
	BuildViewFrustum(Frustum::FRUSTUM_TYPE_EFFECT);
#endif
#ifdef ENABLE_GRAPHIC_OPTIMIZATION
	BuildViewFrustum(Frustum::FRUSTUM_TYPE_TREE);
#endif
#ifdef ENABLE_GRAPHIC_OPTIMIZATION
	BuildViewFrustum(Frustum::FRUSTUM_TYPE_SHOP);
#endif
}

void CScreen::BuildViewFrustum(BYTE frustumType)
{
	...
	const D3DXVECTOR3& c_rv3Target=CCameraManager::Instance().GetCurrentCamera()->GetTarget();
	ms_frustum.BuildViewFrustum2(ms_matView*ms_matProj, ms_fNearY, ms_fFarY, ms_fFieldOfView, ms_fAspect, c_rv3Eye, c_rv3View, frustumType);
}
#else
