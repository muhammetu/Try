...
{
	int dataCodePage = ms_uOutputCodePage;
...
void CIME::PasteTextFromClipBoard()
{
	...
	if (!handle) // @duzenleme eger ctrl v yapamazsam crash yemesin diye.
	{
		CloseClipboard();
		return;
	}
	char* buffer = (char*)GlobalLock(handle);
	if (!buffer) // @duzenleme eger ctrl v yapamazsam crash yemesin diye.
	{
		GlobalUnlock(handle);
		CloseClipboard();
		return;
	...
	if (wstrLen <= 0) // @duzenleme eger ctrl v yapamazsam crash yemesin diye.
		return;
...
void CIME::CopyTextToClipBoard(const char* text)
{
...
void CIME::OnChar(wchar_t c)
{
	...
	if ((c >= 0x00 && c <= 0x1f) || (c == 0x7f)) // @duzenleme bilinmeyen karakterleri ctrl v yapmasin diye.
		return;
