...
bool CGraphicImageTexture::CreateDeviceObjects()
{
	...
	else
	{
		...
		// @duzenleme
		// bazi ddsler exeyi kapattiriyordu.
		if (!CreateFromMemoryFile(mappedFile.Size(), c_pvMap, m_d3dFmt, m_stFileName.c_str(), m_dwFilter))
		{
...
bool CGraphicImageTexture::CreateDDSTexture(CDXTCImage& image, const BYTE* /*c_pbBuf*/)
{
	...
	if (FAILED(D3DXCreateTexture(ms_lpd3dDevice, image.m_nWidth, image.m_nHeight, mipmapCount, 0, format, pool, &lpd3dTexture)))
	{
	...
	else
	{
		...
		if (FAILED(D3DXCreateTexture(ms_lpd3dDevice, imgWidth, imgHeight, mipmapCount, 0, format, D3DPOOL_MANAGED, &m_lpd3dTexture)))
		{
			TraceError("CreateDDSTexture: Cannot creatre texture");
			return false;
		}
...
bool CGraphicImageTexture::CreateFromMemoryFile(UINT bufSize, const void* c_pvBuf, D3DFORMAT d3dFmt, const char* fileName, DWORD dwFilter)
{
	assert(ms_lpd3dDevice != NULL);
	assert(m_lpd3dTexture == NULL);
	...
	else
	{
		...
		if (FAILED(D3DXCreateTextureFromFileInMemoryEx(
			...
			&m_lpd3dTexture)))
		{
			TraceError("CreateFromMemoryFile: Cannot create texture %s", fileName);
			return false;
		}
