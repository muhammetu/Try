...
CCameraManager::CCameraManager() :
	...
	,m_pRenderTargetCamera(NULL)
#endif
{
	AddCamera(DEFAULT_PERSPECTIVE_CAMERA);
	AddCamera(DEFAULT_ORTHO_CAMERA);
#ifdef ENABLE_MODEL_RENDER_TARGET
	AddCamera(DEFAULT_MODEL_RENDER_CAMERA);
#endif

	SetCurrentCamera(DEFAULT_PERSPECTIVE_CAMERA);
#ifdef ENABLE_MODEL_RENDER_TARGET
	SetRenderTargetCamera(DEFAULT_MODEL_RENDER_CAMERA);
#endif
}
...
#ifdef ENABLE_MODEL_RENDER_TARGET
CCamera * CCameraManager::GetRenderTargetCamera()
{
	if (!m_pRenderTargetCamera)
		assert(false);
	return m_pRenderTargetCamera;
}

void CCameraManager::SetRenderTargetCamera(unsigned char ucCameraNum)
{
	m_pRenderTargetCamera = m_CameraMap[ucCameraNum];
}
#endif
