...
void CDibBar::SetColor(DWORD dwColor)
{
	m_dwColor = dwColor;
}
...
void CDibBar::Render(int ix, int iy)
{
	...
	{
		CBlockTexture* pTexture = *itor;
		pTexture->Render(ix, iy, m_dwColor);
	}
}
...
CDibBar::CDibBar() : m_dwWidth(0), m_dwHeight(0), m_dwColor(0xffffffff)
{
}
CDibBar::~CDibBar()
{
	stl_wipe(m_kVec_pkBlockTexture);
}