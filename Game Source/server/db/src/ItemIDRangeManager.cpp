...
void CItemIDRangeManager::Build()
{
	...
	for (int i = 0; ; ++i)
	{
		...
		{
			total += (range.dwMax - range.dwUsableItemIDMin);

			m_listData.push_back(range);
		}
	}

	fprintf(stderr, "ITEM ID RANGE BUILDED (LIST COUNT: %u, TOTAL ID: %u)\n", m_listData.size(), total);
}
...
TItemIDRangeTable CItemIDRangeManager::GetRange()
{
	...
	if (m_listData.size() < 20)
	{
		fprintf(stderr, "!!!Range count decrasing!!! (size: %u)\n", m_listData.size());
	}

	sys_log(0, "Count of Item ID Range: %u", m_listData.size());
...
bool CItemIDRangeManager::BuildRange(DWORD dwMin, DWORD dwMax, TItemIDRangeTable& range)
{
	...
	pMsg = CDBManager::instance().DirectQuery(szQuery, SQL_PLAYER_ITEM);
