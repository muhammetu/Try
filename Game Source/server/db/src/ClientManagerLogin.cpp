...
void CClientManager::QUERY_LOGIN_BY_KEY(CPeer* pkPeer, DWORD dwHandle, TPacketGDLoginByKey* p)
{
	...
	strlcpy(pkTab->cpu_id, r.cpu_id, sizeof(pkTab->cpu_id));
	strlcpy(pkTab->hdd_model, r.hdd_model, sizeof(pkTab->hdd_model));
	strlcpy(pkTab->machine_guid, r.machine_guid, sizeof(pkTab->machine_guid));
	strlcpy(pkTab->mac_addr, r.mac_addr, sizeof(pkTab->mac_addr));
	strlcpy(pkTab->hdd_serial, r.hdd_serial, sizeof(pkTab->hdd_serial));
	strlcpy(pkTab->bios_id, r.bios_id, sizeof(pkTab->bios_id));
	strlcpy(pkTab->pc_name, r.pc_name, sizeof(pkTab->pc_name));
	strlcpy(pkTab->user_name, r.user_name, sizeof(pkTab->user_name));
	strlcpy(pkTab->os_version, r.os_version, sizeof(pkTab->os_version));
...
#ifdef __HWID_SECURITY_UTILITY__
	strlcpy(pkTab->cpu_id, row[col++], sizeof(pkTab->cpu_id));
	strlcpy(pkTab->hdd_model, row[col++], sizeof(pkTab->hdd_model));
	strlcpy(pkTab->machine_guid, row[col++], sizeof(pkTab->machine_guid));
	strlcpy(pkTab->mac_addr, row[col++], sizeof(pkTab->mac_addr));
	strlcpy(pkTab->hdd_serial, row[col++], sizeof(pkTab->hdd_serial));
	strlcpy(pkTab->bios_id, row[col++], sizeof(pkTab->bios_id));
	strlcpy(pkTab->pc_name, row[col++], sizeof(pkTab->pc_name));
	strlcpy(pkTab->user_name, row[col++], sizeof(pkTab->user_name));
	strlcpy(pkTab->os_version, row[col++], sizeof(pkTab->os_version));
#endif // __HWID_SECURITY_UTILITY__
...
#ifdef __CHARACTER_WINDOW_RENEWAL__
					pkTab->players[j].last_play = pt->last_play;
#endif
				}
				else
				{
