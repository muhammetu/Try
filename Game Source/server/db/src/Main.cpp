...
std::string g_stLocaleNameColumn = "locale_name";
...
int g_iQuestCacheFlushSeconds = 60 * 5;
int g_iAffectCacheFlushSeconds = 60 * 5;
#ifdef __SKILL_COLOR__
int g_iSkillColorCacheFlushSeconds = 60 * 7;
#endif
#ifdef __BATTLE_PASS_SYSTEM__
int g_iBattlePassCacheFlushSeconds = 60 * 5;
...
void CalculateQueryCount(size_t& ret)
{
	static std::map<int, size_t> slots;

	for (size_t i = SQL_PLAYER; i < SQL_MAX_NUM; i++)
	{
		slots[i] = CDBManager::instance().CountReturnQuery(i) +
					CDBManager::instance().CountAsyncQuery(i);
	}

	ret = 0;

	for (auto &&e_slot : slots)
	{
		ret += e_slot.second;
	}
}
...
int Start()
{
	...
	if (CConfig::instance().GetValue("AFFECT_CACHE_FLUSH_SECONDS", szBuf, 256))
	{
		str_to_number(g_iAffectCacheFlushSeconds, szBuf);
		sys_log(0, "AFFECT_CACHE_FLUSH_SECONDS: %d", g_iAffectCacheFlushSeconds);
	}
	...
	if (CConfig::instance().GetValue("BATTLE_PASS_CACHE_FLUSH_SECONDS", szBuf, 256))
	{
		str_to_number(g_iBattlePassCacheFlushSeconds, szBuf);
		sys_log(0, "BATTLE_PASS_CACHE_FLUSH_SECONDS: %d", g_iBattlePassCacheFlushSeconds);
	}
#endif
	...
	if (CConfig::instance().GetValue("SQL_PLAYER", line, 256))
	{
		...
		#define DO_CONNECT_PLAYER(sql_name)   \
			iRetry = 5;   \
			do   \
			{   \
				if (CDBManager::instance().Connect(sql_name, szAddr, iPort, szDB, szUser, szPassword))   \
				{   \
					sys_log(0, "Connect " #sql_name " OK");   \
					break;   \
				}   \
		...
		DO_CONNECT_PLAYER(SQL_PLAYER)
		DO_CONNECT_PLAYER(SQL_PLAYER_ITEM)
		DO_CONNECT_PLAYER(SQL_PLAYER_AFFECT)
		DO_CONNECT_PLAYER(SQL_PLAYER_QUEST)
		DO_CONNECT_PLAYER(SQL_PLAYER_SKILLCOLOR)
		DO_CONNECT_PLAYER(SQL_PLAYER_STATISTICS)
		DO_CONNECT_PLAYER(SQL_PLAYER_OPTIONS)
		DO_CONNECT_PLAYER(SQL_PLAYER_GUILD)
		DO_CONNECT_PLAYER(SQL_PLAYER_OFFLINESHOP)
		DO_CONNECT_PLAYER(SQL_PLAYER_SAFEBOX)
		DO_CONNECT_PLAYER(SQL_PLAYER_BATTLE_PASS)
