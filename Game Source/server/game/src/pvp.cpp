...
bool CPVPManager::IsFighting(DWORD dwPID)
{
	...
	while (it2 != it->second.end())
	{
		...
		if (!pkPVP) // @duzenleme eger pvp de kayitli degilse diye koymusuz ama kayitli degilse IsFigting kullanmamasi lazim yani memoryleak var.
			return false;
...
void CPVPManager::Process()
{
	...
	while (it != m_map_pkPVP.end())
	{
		...
		if (g_bEndPvpWhenDisconnect && pvp->IsEndState() == true)
		{
			pvp->Packet(true);
			Delete(pvp);
		}
