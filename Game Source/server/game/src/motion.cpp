...
static void LoadSkillMotion(CMotionSet* pMotionSet, CMob* pMob, EPublicMotion motion)
{
	...
	else
	{
		...
		{
			sys_err("Motion: Skill exist but no motion data for index %d mob %u skill %u file %s",
				   	idx, mob_table->dwVnum, mob_table->Skills[idx].dwVnum, cpFileName);
		}
		M2_DELETE(pMotion);
	}
}
...
bool CMotionManager::Build()
{
	...
	for (int i = 0; i < MAIN_RACE_MAX_NUM; ++i)
	{
		...
		if (i == MAIN_RACE_ASSASSIN_M || i == MAIN_RACE_ASSASSIN_W) {
		...
		if (i == MAIN_RACE_SHAMAN_M || i == MAIN_RACE_SHAMAN_W) {
