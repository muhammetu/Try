...
bool is_gm_account(const char* account)
{
	std::unique_ptr<SQLMsg> _gm_login(DBManager::instance().DirectQuery("SELECT mAccount FROM common.gmlist WHERE mAccount='%s' limit 1;", account));
