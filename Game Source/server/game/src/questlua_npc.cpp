...
namespace quest
{
	...
	ALUA(npc_get_leader_vid)
	{
		...
		if (!party) // @duzenleme eger grup yoksa leaderin victim idini almaya calisiyor bu yuzden core veriyor.
		{
	...
	ALUA(npc_is_boss)
	{
		lua_pushboolean(L, CQuestManager::instance().GetCurrentNPCCharacterPtr()->IsBoss());
		return 1;
	}
