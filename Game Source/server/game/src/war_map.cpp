...
void CWarMap::UsePotion(LPCHARACTER ch, LPITEM item)
{
...
#ifdef __GOLD_LIMIT_REWORK__
	long long llPrice = item->GetProto()->llGold;
#else
	...
	if (gid == m_TeamData[0].dwID)
		m_TeamData[0].llUsePotionPrice += llPrice;
	else if (gid == m_TeamData[1].dwID)
		m_TeamData[1].llUsePotionPrice += llPrice;
#else
	if (gid == m_TeamData[0].dwID)
...
{
	set_pidJoiner.erase(ch->GetPlayerID()); // @duzenleme adam oyundan ciktiginda tabloda guncellenmiyordu onu yaptik.
	--iMemberCount;
}
...
void CWarMap::Timeout()
{
	...
	else
	{
		...
		int team1MemberCount = m_TeamData[0].iMemberCount;
		int team2MemberCount = m_TeamData[1].iMemberCount;

		if (team1MemberCount == 0 && team2MemberCount == 0)
			iWinnerIdx = -1; // Both guilds left the war
		else if (team1MemberCount == 0)
			iWinnerIdx = 1;
		else if (team2MemberCount == 0)
			iWinnerIdx = 0;

		if (iWinnerIdx == -1)
		{
			dwWinner = GetWinnerGuild(); // returns 0 if the score is the same!
			...
			sys_err("WarMap::Timeout: both guilds had no players, guild1 %u %d guild2 %u %d, winner %u", m_TeamData[0].dwID, team1MemberCount, m_TeamData[1].dwID, team2MemberCount, dwWinner);
		}
		else
		{
...
#endif
#ifdef __GUILD_WAR_REWORK__
				, m_TeamData[0].iMaxPlayer, m_TeamData[0].iMaxScore
#endif
			);
	else
#ifdef __GOLD_LIMIT_REWORK__
...
bool CWarMap::CheckScore()
{
	...
	sys_log(0, "WarMap::CheckScore end score %d guild1 %u score %d guild2 %u score %d winner %u",
		iEndScore,
		m_TeamData[0].dwID,
		m_TeamData[0].iScore,
		m_TeamData[1].dwID,
...
void CWarMap::OnKill(LPCHARACTER killer, LPCHARACTER ch)
{
	...
	case WAR_MAP_TYPE_NORMAL:
		SendGuildWarScore(dwKillerGuild, dwDeadGuild, 1);
		break;
