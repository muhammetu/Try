...
namespace fishing
{
	...
	void Initialize()
	{
		if (g_bAuthServer) // @duzenleme auth coresinde balikcilik yapamiyorlar bu yuzden fishing.txt eklenmesine gerek gormedik.
		{
		...
		while (fgets(buf, 512, fp))
		{
			...
			*p = '\0';
			...
			if (idx == MAX_FISH) // @duzenleme eger fishing.txt tablosunun limiti asilir ise gorebilmek icin sys_log eklentisi ekledik.
			{
				sys_log(0, "MAX_FISH %u reached", MAX_FISH);
				break;
			}
		}
		...
		//{
		//	sys_log(0, "FISH: %-64s vnum %5lu prob %4d %4d %4d %4d len %d %d %d",
		//		fish_info[i].name,
		//		fish_info[i].vnum,
		//		fish_info[i].prob[0],
		//		fish_info[i].prob[1],
	...
	LPEVENT CreateFishingEvent(LPCHARACTER ch)
	{
		...
		int time = number(g_iFishingTimeMin, g_iFishingTimeMax);
	...
	void Take(fishing_event_info* info, LPCHARACTER ch)
	{
		if (info->step == 1)
		{
			...
			quest::CQuestManager::instance().Fishing(ch->GetPlayerID());
			...
			case 0:
				if (item_vnum)
				{
					...
					if (bBattlePassId)
					{
						...
						if (CBattlePass::instance().BattlePassMissionGetInfo(bBattlePassId, CATCH_FISH, &dwVnum, &dwCount))
						{
							if (dwVnum == 0 && ch->GetMissionProgress(CATCH_FISH, bBattlePassId) < dwCount)
								ch->UpdateMissionProgress(CATCH_FISH, bBattlePassId, 1, dwCount);
							else if (dwVnum == item_vnum && ch->GetMissionProgress(CATCH_FISH, bBattlePassId) < dwCount)
								ch->UpdateMissionProgress(CATCH_FISH, bBattlePassId, 1, dwCount);
						}
					}
#endif
	...
	{
		if (!rod)// @duzenleme olta yoksa nasil yukseltcen amin evladi..
			return false;
	...
	int RealRefineRod(LPCHARACTER ch, LPITEM item)
	{
		...
		if (!rod) // @duzenleme olta yoksa nasil yukseltcen amin evladi.. fakat yukardaki !item kontrolunu herhalde gotuyle yapiyor oyun xD
			return 0;
		...
		if (number(1, 100) <= rod->GetValue(3))
		{
			...
			if (pkNewItem)
			{
				...
				pkNewItem->__ADD_TO_CHARACTER(ch, TItemPos(INVENTORY, bCell));
				return 1;
			}
		...
		{
#ifdef __MARTY_FISHINGROD_RENEWAL__
			{
