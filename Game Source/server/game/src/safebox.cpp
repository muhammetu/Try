...
CSafebox::CSafebox(LPCHARACTER pkChrOwner, int iSize, long long llGold) : m_pkChrOwner(pkChrOwner), m_iSize(iSize), m_lGold(llGold)
#else
...
bool CSafebox::Add(DWORD dwPos, LPITEM pkItem)
{
	...
	pack.count = (DWORD)pkItem->GetCount();
#else
	...
	pack.evolution = pkItem->GetEvolution();
#endif
...
void CSafebox::ChangeSize(int iSize)
{
	...
	if (pkOldGrid)
	{
		...
		delete pkOldGrid;// Burada deponuzun boyutu deðiþtiðinde pkOldGrid 'in bellekten silinmesi saðlandý.
	}
	else
		m_pkGrid = M2_NEW CGrid(5, m_iSize);
}
...
#ifdef __ITEM_COUNT_LIMIT__
bool CSafebox::MoveItem(BYTE bCell, BYTE bDestCell, DWORD count)
#else
...
#endif
{
	...
	if (bCell == bDestCell) // @burada collossus kardeþimizin scriptle yapýlan kopyalama fixi yapýldý.
		return false;
