...
EVENTINFO(TSwitchbotEventInfo)
{
	...
	TSwitchbotEventInfo() : pkSwitchbot(NULL), m_fSpeed(0.08f)
	{
	}
};
...
EVENTFUNC(switchbot_event)
{
	...
	return PASSES_PER_SEC(info->m_fSpeed);
}
...
void CSwitchbot::Start()
{
	...
	m_pkSwitchEvent = event_create(switchbot_event, info, m_fSpeed);
...
void CSwitchbot::SetSpeed(BYTE speed)
{
	const float fSpeed[] = {
		0.20f,
		0.16f,
		...
		else
		{
			...
			if (SWITCHBOT_PRICE_TYPE == 1)
			{
				...
				for (const auto& itemVnum : c_arSwitchingItems)
				{
					...
					LPITEM pkItem2 = pkOwner->FindSpecifyItemSpecial(itemVnum);
#else
					LPITEM pkItem2 = pkOwner->FindSpecifyItem(itemVnum);
#endif

#ifdef __BATTLE_PASS_SYSTEM__
					BYTE bBattlePassId = pkOwner->GetBattlePassId();
					if (bBattlePassId && pkItem2)
					{
						DWORD dwItemVnum, dwUseCount;
						if (CBattlePass::instance().BattlePassMissionGetInfo(bBattlePassId, USE_ITEM, &dwItemVnum, &dwUseCount))
						{
							if (dwItemVnum == pkItem2->GetVnum() && pkOwner->GetMissionProgress(USE_ITEM, bBattlePassId) < dwUseCount)
								pkOwner->UpdateMissionProgress(USE_ITEM, bBattlePassId, 1, dwUseCount);
						}
					}
#endif

					if (pkItem2 && !removedItem)
						pkItem2->SetCount(pkItem2->GetCount() - SWITCHBOT_PRICE_AMOUNT);
				}
			}
			else if (SWITCHBOT_PRICE_TYPE == 2)
			{
...
void CSwitchbotManager::ChangeSpeed(DWORD player_id, BYTE speed)
{
	CSwitchbot* pkSwitchbot = FindSwitchbot(player_id);
	if (!pkSwitchbot)
		return;

	pkSwitchbot->SetSpeed(speed);
}
