...
)
{
	...
	case GUILD_WAR_RECV_DECLARE:
	{
		...
		db_clientdesc->DBPacket(HEADER_GD_GUILD_WAR, 0, &p, sizeof(p));
#ifdef __GUILD_WAR_REWORK__
		sys_log(0, "GuildWar.WaitStartSendToDB id(%d vs %d), type(%d), map_index(%d) max_player(%d) max_score(%d)",
			GetID(), dwOppGID, saved_type, guildWarInfo.lMapIndex, saved_maxplayer, saved_maxscore);
#else
		sys_log(0, "GuildWar.WaitStartSendToDB id(%d vs %d), type(%d), map_index(%d)",
...
#ifdef __GUILD_LEADER_DIRECT_GOTO_WAR__
		CGuild* pkGuild = ch->GetGuild();
		if (pkGuild->GetMasterPID() == ch->GetPlayerID())
		{
			GuildWarEntryAccept(dwOppGID, ch);
			break;
		}
#endif
