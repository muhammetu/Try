...
bool DSManager::PullOut(LPCHARACTER ch, TItemPos DestCell, LPITEM& pItem, LPITEM pExtractor)
{
	...
	if (!IsValidCellForThisItem(pItem, DestCell) || ch->GetItem(DestCell)) // @DIKKAT @duzenleme simyayi cikarirken simya envanterinde esya mevcut ise diye koontrol konulmus ama eger esya mevcut degilse yapilmasi gerekli
	{
...

	{
		...
		{
			pItem->__ADD_TO_CHARACTER(ch, DestCell);
			return true;
		}
...
bool DSManager::DoRefineGrade(LPCHARACTER ch, TItemPos(&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	*/
	...
	while (++it != set_items.end())
	{
		...
		if (!pItem) // @duzenleme buraya konmamasi lazim bir sakatlik varki koymusuz.
			return false;
	...
	if (count != need_count)
	{
		sys_log(0, "Possiblity of invalid client. Name %s", ch->GetName());
...
bool IsDragonSoulRefineMaterial(LPITEM pItem)
{
	...
	return (pItem->GetSubType() == MATERIAL_DS_REFINE_NORMAL ||
		...
		pItem->GetSubType() == MATERIAL_DS_REFINE_1 ||
		pItem->GetSubType() == MATERIAL_DS_REFINE_2 ||
		pItem->GetSubType() == MATERIAL_DS_REFINE_3 ||
		pItem->GetSubType() == MATERIAL_DS_REFINE_4 ||
		pItem->GetSubType() == MATERIAL_DS_REFINE_5
#endif
		);
}
...
int DSManager::GetDSSetValue(uint8_t iAttributeIndex, uint16_t iApplyType, DWORD iVnum, uint8_t iSetGrade)
{
