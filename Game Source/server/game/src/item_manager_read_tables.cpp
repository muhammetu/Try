		...
		else
		{
			...
			for (int k = 1; k < 1024; ++k)
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					sys_log(0, "        name %s count %d prob %lf rare %d", name.c_str(), iCount, iProb, iRarePct);
	...
	for (DWORD i = 0; i < loader.GetChildNodeCount(); ++i)
	{
		...
		for (int k = 1; k < 1024; ++k)
		{
			...
			if (loader.GetTokenVector(buf, &pTok))
			{
				...
				// TODO: Buradaki cince yazilarin kaldirimasi gerekiyor cunku kullanmiyoruz.
				if (!GetVnumByOriginalName(name.c_str(), dwVnum))
				{
					if (
						name == "exp" ||
...
#endif
{
	...
	while (fgets(buf, 512, fp))
	{
		...
		*cpTab = '\0';
		...
		{
			str_to_number(dwItemVnum, szItemName); // @duzenleme esyanin ismine gore bulamayince koduna gore ariyor.
			if (!ITEM_MANAGER::instance().GetTable(dwItemVnum))
			{
...
#endif
{
	...
	for (DWORD i = 0; i < loader.GetChildNodeCount(); ++i)
	{
		...
		else if (strType == "drop")
		{
			...
			if (isReloading)
			{
				itertype(m_map_pkDropItemGroup) it = tempDropItemGr.find(iMobVnum);
			...
			for (int k = 1; k < 1024; ++k)
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					sys_log(0, "        name %s pct %f count %d", name.c_str(), fPercent, iCount);
					pkGroup->AddItem(dwVnum, fPercent, iCount);
					continue;
				}
