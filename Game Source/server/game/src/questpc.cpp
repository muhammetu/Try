...
namespace quest
{
	...
	void PC::SetFlag(const string& name, int value, bool bSkipSave)
	{
		...
		// else
			// sys_log(0, "QUEST Setting flag %s %d %d", name.c_str(), value, bSkipSave);
	...
	void PC::SaveFlag(const string& name, int value)
	{
		if (!IsLoaded())
		{
			sys_err("quest::PC::Save : not loaded (%s %d)", name.c_str(), value);
			return;
		}

		sys_log(0, "QUEST SaveFlag %s %d %d", name.c_str(), value);
		...
		string stName;
		stName.assign(name, 0, iPos);

		string stState;
		stState.assign(name, iPos + 1, name.length());

		static TQuestTable r;
		...
		db_clientdesc->DBPacketHeader(HEADER_GD_QUEST_SAVE, 0, sizeof(TQuestTable));
		db_clientdesc->Packet(&r, sizeof(TQuestTable));
	...
	DWORD PC::GetTimerRem(const string& name)
	{
		TTimerMap::iterator it = m_TimerMap.find(name);

		if (it != m_TimerMap.end())
		{
			sys_log(0, "QUEST GetTimerRem timer %p", get_pointer(it->second));
	...
	void PC::SendQuestInfoPakcet()
	{
		...
		int extraSize = 0;
		int extraSizeTitle = 30;
		int extraSizeClockName = 16;
		int extraSizeCounterName = 16;
		int extraSizeIconFile = 24;

		if (m_iSendToClient & QUEST_SEND_ISBEGIN) extraSize += 1;
		if (m_iSendToClient & QUEST_SEND_TITLE) extraSize += extraSizeTitle + 1;
		if (m_iSendToClient & QUEST_SEND_CLOCK_NAME) extraSize += extraSizeClockName + 1;
		if (m_iSendToClient & QUEST_SEND_CLOCK_VALUE) extraSize += 4;
		if (m_iSendToClient & QUEST_SEND_COUNTER_NAME) extraSize += extraSizeCounterName + 1;
		if (m_iSendToClient & QUEST_SEND_COUNTER_VALUE) extraSize += 4;
		if (m_iSendToClient & QUEST_SEND_ICON_FILE) extraSize += extraSizeIconFile + 1;

		qi.header = HEADER_GC_QUEST_INFO;
		qi.size = sizeof(struct packet_quest_info) + extraSize;
		...
		{
			m_RunningQuestState->_title.reserve(extraSizeTitle + 1);
			buf.write(m_RunningQuestState->_title.c_str(), extraSizeTitle + 1);

			sys_log(1, "QUEST Title %s", m_RunningQuestState->_title.c_str());
		}
		if (m_iSendToClient & QUEST_SEND_CLOCK_NAME)
		{
			m_RunningQuestState->_clock_name.reserve(extraSizeClockName + 1);
			buf.write(m_RunningQuestState->_clock_name.c_str(), extraSizeClockName + 1);
		...
		{
			m_RunningQuestState->_counter_name.reserve(extraSizeCounterName + 1);
			buf.write(m_RunningQuestState->_counter_name.c_str(), extraSizeCounterName + 1);
		...
		{
			m_RunningQuestState->_icon_file.reserve(extraSizeIconFile + 1);
			buf.write(m_RunningQuestState->_icon_file.c_str(), extraSizeIconFile + 1);
	...
	void PC::EndRunning()
	{
		...
		//Think - 23/03/14 - Fix items being "remembered" on other quest states
		if (CQuestManager::instance().GetCurrentItem() != NULL)
	...
	{
		return;
		// cache zaten adam oyuna girirince dbye kaydediliyor
		// farklilik olunca da anlik oalrak dbye gonderiyoruz
		// doalyiisyla son durumun dbye aktarilmasina gerek yok
		...
		while (it != m_FlagMap.end())
		{
			...
			sys_log(0, "QUEST Save Flag %s, %s %d (%d)", stName.c_str(), stState.c_str(), lValue, i);
