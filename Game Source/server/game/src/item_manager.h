...
public:
	...
	void AddItem(DWORD vnum, int count, float prob, int rare)
	{
		if (!prob)
			return;
		if (!m_vecProbs.empty())
	...
	int GetMultiIndex(std::vector <int>& idx_vec, DWORD plusPerc = 0) const
#else
	int GetMultiIndex(std::vector <int>& idx_vec) const
#endif // __CHEST_DROP_POINT__
	{
		idx_vec.clear();
		if (m_bType == PCT)
		{
			int count = 0;
#ifdef __CHEST_DROP_POINT__
			if (fnumber(1, 100) <= m_vecProbs[0] + plusPerc)
#else
			if (fnumber(1, 100) <= m_vecProbs[0])
#endif
			{
				idx_vec.push_back(0);
				count++;
			...
			{
#ifdef __CHEST_DROP_POINT__
				if (fnumber(1, 100) <= m_vecProbs[i] - m_vecProbs[i - 1] + plusPerc)
#else
				if (fnumber(1, 100) <= m_vecProbs[i] - m_vecProbs[i - 1])
#endif
				{
					idx_vec.push_back(i);
					count++;
		...
		{
#ifdef __CHEST_DROP_POINT__
			idx_vec.push_back(GetOneIndex(plusPerc));
#else
...
#ifdef __CHEST_DROP_POINT__
	int GetOneIndex(int plusPerc = 0) const
#else
...
#ifdef __CHEST_DROP_POINT__
		float n;
		if (number(1, 100) <= plusPerc)
			n = fnumber(0, m_vecProbs.back()/4);
		else
			n = fnumber(0, m_vecProbs.back());
#else
		float n = fnumber(1, m_vecProbs.back());
#endif
...
public:
	...
	bool			FlushDelayedSave(LPITEM item);
	...
	bool			CreateDropItemVector(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<TMobInformationDropItem>& vec_item);
#endif
...
#ifdef __CHEST_INFO_SYSTEM__
	void 			GetChestItemList(const DWORD& dwChestVnum, std::vector<TChestDropInfoTable>& vec_item);
#endif
