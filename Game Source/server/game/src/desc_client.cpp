			...
			if (p.dwLoginCount)
			{
				...
				for (it = c_set.begin(); it != c_set.end(); ++it)
				{
					...
					if (r.id != 0)
					{
						...
						strlcpy(pck.cpu_id, r.cpu_id, sizeof(pck.cpu_id));
						strlcpy(pck.hdd_model, r.hdd_model, sizeof(pck.hdd_model));
						strlcpy(pck.machine_guid, r.machine_guid, sizeof(pck.machine_guid));
						strlcpy(pck.mac_addr, r.mac_addr, sizeof(pck.mac_addr));
						strlcpy(pck.hdd_serial, r.hdd_serial, sizeof(pck.hdd_serial));
						strlcpy(pck.bios_id, r.bios_id, sizeof(pck.bios_id));
						strlcpy(pck.pc_name, r.pc_name, sizeof(pck.pc_name));
						strlcpy(pck.user_name, r.user_name, sizeof(pck.user_name));
						strlcpy(pck.os_version, r.os_version, sizeof(pck.os_version));
...
bool CLIENT_DESC::SetForceNorm(bool f)
{
	ResetForces();
...
bool CLIENT_DESC::IsForceNorm()
{
	return m_bForceNorm;
}

bool CLIENT_DESC::SetForceBusy(bool f)
{
	ResetForces();
...
bool CLIENT_DESC::IsForceBusy()
{
	return m_bForceBusy;
}

bool CLIENT_DESC::SetForceFull(bool f)
{
	ResetForces();
...
void CLIENT_DESC::UpdateChannelStatus(DWORD t, bool fForce)
{
	...
	if (fForce || tLCSUP < t) {
		...
		if (IsForceNorm())
		{
			channelStatus.bStatus = 1;
		}
		else if (IsForceBusy())
		{
			channelStatus.bStatus = 2;
		}
		else if (IsForceFull())
		{
			channelStatus.bStatus = 3;
		}
		else
		{
			if (g_bNoMoreClient)
