...
{
	return quest::CQuestManager::instance().GetEventFlag("guild_disband_delay") * (test_server ? 60 : 600);
}

int CGuildManager::GetWithdrawDelay()
{
	return quest::CQuestManager::instance().GetEventFlag("guild_withdraw_delay") * (test_server ? 60 : 600);
}
...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
	...
	// @duzenleme
	// lonca uzerinden yapilan sql inject fixi
...
void CGuildManager::RequestWarOver(DWORD dwGuild1, DWORD dwGuild2, DWORD dwGuildWinner
...
#endif
	db_clientdesc->DBPacket(HEADER_GD_GUILD_WAR, 0, &p, sizeof(p));
	sys_log(0, "RequestWarOver : winner %u loser %u draw %u", p.dwGuildFrom, p.dwGuildTo, p.bType);
}
...
#ifdef __GUILD_WAR_REWORK__
	if (g1->WaitStartWar(guild_id2, iMaxPlayer, iMaxScore) ||
		g2->WaitStartWar(guild_id1, iMaxPlayer, iMaxScore))
#else
...
void SendGuildWarScore(DWORD dwGuild, DWORD dwGuildOpp, int iDelta)
{
...
{
	// @duzenleme eger ajansan ve sadece yancarla gecerli olmak uzere xD cari kesiyor isen loncaya puan vermiyor.
