...
ACMD(do_emotion)
{
	...
	// @duzenleme
	// eger sersemlik yemis ise karakter duygulari kullanamiyor.
	if (ch->IsAffectFlag(AFF_STUN) || (victim && victim->IsAffectFlag(AFF_STUN)))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can't use that while you or your victim is stunned."));
		return;
	}
