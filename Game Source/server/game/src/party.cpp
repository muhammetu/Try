...
void CParty::ComputeRolePoint(LPCHARACTER ch, BYTE bRole, bool bAdd)
{
	...
	{
		// @duzenleme
		// size ozellik saglayan grup uyesi uzaklastiginde oyunu laga sokuyordu veya compute oldugunuzda
		// bu yuzden boyle birsey yaptik alinan yer (Lysium2) fakat Hikmet'te kullaniyor bunu kendi oyununda.
		bool cRefresh = false;
#ifdef __PARTY_ROLE_REWORK__
		for (auto pPoints : { POINT_PARTY_ATTACKER_BONUS, POINT_PARTY_TANKER_BONUS, POINT_PARTY_BUFFER_BONUS, POINT_PARTY_SKILL_MASTER_BONUS, POINT_PARTY_DEFENDER_BONUS, POINT_PARTY_HASTE_BONUS, POINT_PARTY_ATTACKER_MONSTER_BONUS, POINT_PARTY_ATTACKER_STONE_BONUS, POINT_PARTY_ATTACKER_BOSS_BONUS })
#else
		for (auto pPoints : { POINT_PARTY_ATTACKER_BONUS, POINT_PARTY_TANKER_BONUS, POINT_PARTY_BUFFER_BONUS, POINT_PARTY_SKILL_MASTER_BONUS, POINT_PARTY_DEFENDER_BONUS, POINT_PARTY_HASTE_BONUS })
#endif
		{
	...
	{
		int iBonus = 10;
		if (ch->GetPoint(POINT_PARTY_ATTACKER_MONSTER_BONUS) != iBonus)
		{
			ch->PointChange(POINT_PARTY_ATTACKER_MONSTER_BONUS, iBonus - ch->GetPoint(POINT_PARTY_ATTACKER_MONSTER_BONUS));
			ch->ComputePoints();
		}
	}
	break;
	...
	{
		int iBonus = 10;
		if (ch->GetPoint(POINT_PARTY_ATTACKER_STONE_BONUS) != iBonus)
		{
			ch->PointChange(POINT_PARTY_ATTACKER_STONE_BONUS, iBonus - ch->GetPoint(POINT_PARTY_ATTACKER_STONE_BONUS));
			ch->ComputePoints();
		}
	}
	break;
	...
	{
		int iBonus = 10;
		if (ch->GetPoint(POINT_PARTY_ATTACKER_BOSS_BONUS) != iBonus)
		{
			ch->PointChange(POINT_PARTY_ATTACKER_BOSS_BONUS, iBonus - ch->GetPoint(POINT_PARTY_ATTACKER_BOSS_BONUS));
			ch->ComputePoints();
		}
	}
	break;
...
#ifdef __PARTY_ROLE_REWORK__
	int iMaxRoleNum = PARTY_MAX_MEMBER;
#else
	int iMaxRoleNum = 1;
#endif

	m_anMaxRole[PARTY_ROLE_ATTACKER] = m_iLeadership >= 10 ? iMaxRoleNum : 0;
	m_anMaxRole[PARTY_ROLE_HASTE] = m_iLeadership >= 10 ? iMaxRoleNum : 0; // default 20
	m_anMaxRole[PARTY_ROLE_TANKER] = m_iLeadership >= 10 ? iMaxRoleNum : 0; // default 20
	m_anMaxRole[PARTY_ROLE_BUFFER] = m_iLeadership >= 15 ? iMaxRoleNum : 0; // default 25
	m_anMaxRole[PARTY_ROLE_SKILL_MASTER] = m_iLeadership >= 25 ? iMaxRoleNum : 0; // default 35
	m_anMaxRole[PARTY_ROLE_DEFENDER] = m_iLeadership >= 30 ? iMaxRoleNum : 0; // default 30
	m_anMaxRole[PARTY_ROLE_ATTACKER] += m_iLeadership >= 30 ? iMaxRoleNum : 0; // default 30
#ifdef __PARTY_ROLE_REWORK__
	m_anMaxRole[PARTY_ROLE_ATTACKER_MONSTER] = m_iLeadership >= 40 ? 1 : 0; //
	m_anMaxRole[PARTY_ROLE_ATTACKER_STONE] = m_iLeadership >= 40 ? 1 : 0; //
	m_anMaxRole[PARTY_ROLE_ATTACKER_BOSS] = m_iLeadership >= 40 ? 1 : 0; //
#endif
...
int CParty::ComputePartyBonusExpPercent()
{
	...
	// UPGRADE_PARTY_BONUS
#ifdef __PARTY_ROLE_REWORK__
	int iMemberCount = MIN(12, GetNearMemberCount());
#else
