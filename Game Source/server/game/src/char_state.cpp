...
void CHARACTER::__StateIdle_Stone()
{
	...
	if (GetLastAttacked() > get_global_time() + 5)
	{
		SetHasAttacker(false);
		SetNormalPlayer(NULL);
	}
...
void CHARACTER::__StateIdle_Monster()
{
	...
	if (!victim)
	{
		// Stone protection processing
		if (m_pkChrStone)
			victim = m_pkChrStone->GetNearestVictim(m_pkChrStone);
		// Pre-monster processing
...
void CHARACTER::StateMove()
{
	...
	else
	{
		...
		{
			LPCHARACTER victim = GetVictim();
			if (victim && !victim->IsInSafezone()) //@duzenleme eger victim guvenli bolgedeyse diye eklendi ama calistigini dusunmuyorum.
		...
		if (IsMonster() && GetMobRank() >= MOB_RANK_BOSS && GetVictim())
		{
			...
			// Eger col kaplumbagasi gibi bir mob yaratilmak isteniyorsa
			// asagida 2192 yazan yerin yanina kodun girilmesi yeterlidir.
...
void CHARACTER::StateBattle()
{
	...
	{
		if (victim && victim->IsDead() && IsAggressive() && (!GetParty() || GetParty()->GetLeader() == this))
		{
	...
	if (victim) // @duzenleme burada victim varsa diye yapmisiz fakat elseye birsey koymayi unutmustuk bu Sync ile alakali problemler yaratabilir duzeltildi.
	{
		if (!Attack(victim))
...
{
	float	START_FOLLOW_DISTANCE = 163.0f; // @bilinmeyenduzenleme
	float	START_RUN_DISTANCE = 575.0f; // @bilinmeyenduzenleme
