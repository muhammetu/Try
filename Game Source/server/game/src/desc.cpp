...
#ifndef __IMPROVED_PACKET_ENCRYPTION__
	thecore_memcpy(m_adwEncryptionKey, "Xg?Gp.qP(SSu~jsg2e*$a>Q[", sizeof(DWORD) * 4);
	thecore_memcpy(m_adwDecryptionKey, "Xg?Gp.qP(SSu~jsg2e*$a>Q[", sizeof(DWORD) * 4);
#endif
...
void DESC::FlushOutput()
{
	...
	while (buffer_size(m_lpOutputBuffer) > 0)
	{
		...
		if (iSecondsPassed > 10)
		{
			...
			{
				sys_log(0, "DESC::FlushOutput: SetPhase PHASE_CLOSE");
				SetPhase(PHASE_CLOSE);
				break;
			}
		}
