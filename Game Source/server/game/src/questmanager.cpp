...
namespace quest
{
	...
	bool CQuestManager::Initialize()
	{
		...
		m_mapEventName.insert(TEventNameMap::value_type("fishing", QUEST_FISHING_EVENT));
...
#ifdef __SCP1453_EXTENSIONS__
		SetEventFlag("zindanlar_aktif", 1);
#endif
	...
	void CQuestManager::Input(unsigned int pc, const char* msg)
	{
		...
		//	if (!isalnum(msg[i]))
		//	{
		//		// sys_err("quest not valid input (msg: %s) (pid: %d)", msg, GetCurrentCharacterPtr()->GetPlayerID());
		//		r_msg = "";
		//		break;
		//	}
		//}
		...
		if ((pPC = GetPC(pc)) && pPC->IsRunning() && pPC->GetRunningQuestState()->suspend_state == SUSPEND_STATE_SELECT)
		{
			...
			if (!pPC->GetRunningQuestState()->chat_scripts.empty())
			{
				...
				else
				{
					...
					// if (pas->when_condition.size() != 0)
					// {
						// if (IsScriptTrue(&pas->when_condition[0], pas->when_condition.size()))
						// {
						...
						// 	if (ch)
						// 	{
						// 		LogManager::instance().HackLog("Quest when condition bypass dedected!", ch);
						// 		LogManager::instance().HackLog(&pas->when_condition[0], ch);
	...
	void CQuestManager::Kill(unsigned int pc, unsigned int npc)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			...
			// kill call script
			if (npc >= MAIN_RACE_MAX_NUM) //@duzenleme eger karakter oldururseniz when kill fonksiyonu 2 kere calisiyordu.
				m_mapNPC[npc].OnKill(*pPC); //@warme004
			m_mapNPC[QUEST_NO_NPC].OnKill(*pPC);

#ifdef __MARTY_PARTYKILL__
...
#ifdef __SCP1453_EXTENSIONS__
	int CQuestManager::GetServerTimerRemainTime(const std::string& name, DWORD arg)
	{
		itertype(m_mapServerTimer) it = m_mapServerTimer.find(make_pair(name, arg));
		if (it != m_mapServerTimer.end())
		{
			LPEVENT event = it->second;
			return (event_time(event) / passes_per_sec);
		}
		return 0;
	}
#endif

	void CQuestManager::Fishing(unsigned int pc)
	{
		PC * pPC;
		if ((pPC = GetPC(pc)))
		{
			if (!CheckQuestLoaded(pPC))
				return;
			m_mapNPC[QUEST_NO_NPC].OnFishing(*pPC);
		}
		else
			sys_err("QUEST no such pc id : %d", pc);
	}
	...
	void CQuestManager::SetEventFlag(const string& name, int value)
	{
...
#ifdef __EVENT_SYSTEM__
		else if (name == "x2metin_event")
			CGameEventsManager::instance().SetActivateEvent(EVENT_DOUBLEMETIN, value != 0 ? true : false);
		else if (name == "x2boss_event")
			CGameEventsManager::instance().SetActivateEvent(EVENT_DOUBLEBOSS, value != 0 ? true : false);
		else if (name == "x2hasar_event")
			CGameEventsManager::instance().SetActivateEvent(EVENT_MAXDAMAGE, value != 0 ? true : false);
		else if (name == "x2duyuru_event")
			CGameEventsManager::instance().SetActivateEvent(EVENT_X2DUYURU, value != 0 ? true : false);
		else if (name == "x2sure_event")
			CGameEventsManager::instance().SetActivateEvent(EVENT_X2SURE, value != 0 ? true : false);
		else if (name == "yukseltme_event")
			CGameEventsManager::instance().SetActivateEvent(EVENT_UPGRADE, value != 0 ? true : false);
#endif
#ifdef ENABLE_FULL_SYSTEM
		// bu kodu daha kisa for loop ile yazardim kankada vaktimiz yok simdi bide else koyarsam bos event flaglarida orda islicek
		// ugrasmak istemedim cok performans sikintisi yaratmaz
		else if (name == "force_norm_ch1" && g_bChannel == 1)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
					db_clientdesc->UpdateChannelStatus(0, true);
			}
			else
			{
				if (db_clientdesc->SetForceNorm(true))
					db_clientdesc->UpdateChannelStatus(0, true);
			}
		}
		else if (name == "force_norm_ch2" && g_bChannel == 2)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_norm_ch3" && g_bChannel == 3)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_norm_ch4" && g_bChannel == 4)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_norm_ch5" && g_bChannel == 5)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_norm_ch6" && g_bChannel == 6)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_norm_ch7" && g_bChannel == 7)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_norm_ch8" && g_bChannel == 8)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		// busy
		else if (name == "force_busy_ch1" && g_bChannel == 1)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
			...
			{
				if (db_clientdesc->SetForceBusy(true))
					db_clientdesc->UpdateChannelStatus(0, true);
			}
		}
		else if (name == "force_busy_ch2" && g_bChannel == 2)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_busy_ch3" && g_bChannel == 3)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_busy_ch4" && g_bChannel == 4)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_busy_ch5" && g_bChannel == 5)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_busy_ch6" && g_bChannel == 6)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_busy_ch7" && g_bChannel == 7)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_busy_ch8" && g_bChannel == 8)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		// full
		else if (name == "force_full_ch1" && g_bChannel == 1)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
			...
			{
				if (db_clientdesc->SetForceFull(true))
					db_clientdesc->UpdateChannelStatus(0, true);
			}
		}
		else if (name == "force_full_ch2" && g_bChannel == 2)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_full_ch3" && g_bChannel == 3)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_full_ch4" && g_bChannel == 4)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_full_ch5" && g_bChannel == 5)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_full_ch6" && g_bChannel == 6)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_full_ch7" && g_bChannel == 7)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
		...
		else if (name == "force_full_ch8" && g_bChannel == 8)
		{
			if (value == 0)
			{
				if (db_clientdesc->ResetForces())
	...
	void CQuestManager::QuestError(const char* func, int line, const char* fmt, ...)
	{
		...
		_sys_err(func, line, "quest(%s) %s", GetCurrentQuestName().c_str(), szMsg);
		if (test_server)
		{
			LPCHARACTER ch = GetCurrentCharacterPtr();
			if (ch)
	...
	void CQuestManager::AddServerTimer(const std::string& name, DWORD arg, LPEVENT event)
	{
		...
		LPDUNGEON pDungeon = CDungeonManager::instance().FindByMapIndex(arg);
		if (pDungeon)
		{
			// char buf[256];
			// snprintf(buf, sizeof(buf), "AddServerTimer:: name %s", name.c_str());
			// pDungeon->AddNoticeLog(buf);
		}
		else
		{
			sys_err("AddServerTimer No Dungeon (name %s)", name.c_str());
		}
	...
	void CQuestManager::ClearServerTimerNotCancel(const std::string& name, DWORD arg)
	{
		...
		{
			// char buf[256];
			// snprintf(buf, sizeof(buf), "ClearServerTimerNotCancel:: name %s", name.c_str());
			// pDungeon->AddNoticeLog(buf);
		}
		else
		{
	...
	void CQuestManager::ClearServerTimer(const std::string& name, DWORD arg)
	{
		...
		{
			// char buf[256];
			// snprintf(buf, sizeof(buf), "ClearServerTimer:: name %s", name.c_str());
			// pDungeon->AddNoticeLog(buf);
		}
		else
		{
