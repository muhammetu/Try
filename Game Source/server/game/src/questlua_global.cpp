...
namespace quest
{
	...
	ALUA(_notice_improving)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_NOTICE_IMPROVING, "%s", s.str().c_str());
		return 0;
	}
#endif
	...
	ALUA(_clear_server_timer)
	{
		...
		if (!lua_isnumber(L, 2))
		{
			sys_err("server_timer with no arg %s", name);
			return 0;
		}
		DWORD arg = (DWORD)lua_tonumber(L, 2); // @bilinmeyenduzenleme
		if (arg == 0)
		{
			sys_err("server_timer with zero arg %s", name);
			return 0;
		}
		q.ClearServerTimer(name, arg);
		return 0;
	...
	ALUA(_get_server_timer_rem)
	{
		...
		lua_pushnumber(L, q.GetServerTimerRemainTime(name, arg));
		return 1;
	}
	ALUA(_get_timer_rem)
	{
		...
		CQuestManager& q = CQuestManager::instance();
		lua_pushnumber(L, q.GetCurrentPC()->GetTimerRem(name));
		return 1;
	}
#endif
...
#ifdef __CHAT_FILTER__
	ALUA(_notice_all_improving)
	{
		ostringstream s;
		combine_lua_string(L, s);

		TPacketGGNotice p;
		p.bHeader = HEADER_GG_NOTICE_IMPROVING;
		p.lSize = strlen(s.str().c_str()) + 1;
		...
		P2P_MANAGER::instance().Send(buf.read_peek(), buf.size()); // HEADER_GG_NOTICE_IMPROVING

		SendNoticeImproving(s.str().c_str());
		return 1;
	}
#endif
...
#define NAN (INFINITY-INFINITY)
	...
	ALUA(_get_map_base_x)
	{
		...
		LPSECTREE_MAP pkSectreeMap = SECTREE_MANAGER::instance().GetMap(lua_tonumber(L, 1));
		if (!pkSectreeMap)
		{
			sys_err("cannot find map by index %d", lua_tonumber(L, 1));
			return 0;
		}
		else {
			lua_pushnumber(L, pkSectreeMap->m_setting.iBaseX);
			return 1;
		}
	}
	ALUA(_get_map_base_y)
	{
		if (!lua_isnumber(L, 1))
		{
			sys_err("wrong argument");
		...
		if (!pkSectreeMap)
		{
			...
			lua_pushnumber(L, pkSectreeMap->m_setting.iBaseY);
			return 1;
		}
	}
...
/*#ifdef __GAME_OPTION_DLG_RENEWAL__
		std::string dungeonAdmin = "Zindan-Admin";
		if ((findPC->GetGameOptionValue(OPTION_HIDE_DUNGEON) == 1) && dungeonAdmin != std::string::npos)
			return 0;

		std::string dungeonBilgi = "Zindan Bilgi";
		if ((findPC->GetGameOptionValue(OPTION_HIDE_DUNGEON) == 1) && dungeonBilgi != std::string::npos)
			return 0;

		std::string dungeonReminder = "Zindan Hatirlaticisi";
		if ((findPC->GetGameOptionValue(OPTION_HIDE_DUNGEON) == 1) && dungeonReminder != std::string::npos)
			return 0;

		std::string biologReminder = "Biyolog Hatirlaticisi";
		if ((findPC->GetGameOptionValue(OPTION_HIDE_BIOLOG) == 1) && biologReminder != std::string::npos)
			return 0;
#endif*/
	...
	ALUA(_is_map_allow)
	{
		lua_pushboolean(L, map_allow_find((int) lua_tonumber(L, 1)));
		return 1;
	}
...
#ifdef __SCP1453_EXTENSIONS__
			{	"get_server_timer_rem",		_get_server_timer_rem	},
			{	"get_timer_rem",			_get_timer_rem			},
#endif
			{	"timer",					_timer					},
...
#endif
#ifdef __SCP1453_EXTENSIONS__
			{	"get_map_base_x",			_get_map_base_x				},
			{	"get_map_base_y",			_get_map_base_y				},
			{	"send_whisper",				_send_whisper				},
#endif
#ifdef __CHAT_FILTER__
			{	"notice_improving",			_notice_improving				},
			{	"notice_all_improving",		_notice_all_improving			},
#endif
			{	"hack_log_ex",				_hack_log_ex					},
			{	"is_map_allow",				_is_map_allow					},
			{	NULL,	NULL	}
		};
