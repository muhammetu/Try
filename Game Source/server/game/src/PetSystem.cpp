...
void CPetActor::SetName()
{
	...
	if (0 != m_pkOwner && 0 != m_pkOwner->GetName())
		snprintf(buf, sizeof(buf), "%s - Pet", m_pkOwner->GetName());
	else
		snprintf(buf, sizeof(buf), "Pet");
...
DWORD CPetActor::Summon(const char* petName, LPITEM pSummonItem, bool bSpawnFar)
{
	...
	DWORD dwRes = pSummonItem->GetTransmutation() != 0 ? pSummonItem->GetTransmutation() : m_dwVnum;
...
DWORD CPetSystem::GetSummonItemVID()
{
	DWORD itemid = 0;
	for (TPetActorMap::iterator iter = m_petActorMap.begin(); iter != m_petActorMap.end(); ++iter)
	{
		...
		{
			if (petActor->IsSummoned()) {
				itemid = petActor->GetSummonItemVID();
				break;
			}
		}
	}
