...
CGuild::CGuild(TGuildCreateParameter& cp)
{
	...
	if (pmsg->Get()->uiAffectedRows < 1) // @duzenleme loncayi kuramazsa devam etmiyor cunku insertID mevcut degil ise crash yeriz.
	{
		sys_err("Failed To Create Guild %s", cp.name);
		return;
	}
...
void CGuild::Initialize()
{
	...
	// @duzenleme buradaki kontrol karaktere ozel yapilmali loncaya degil
	m_guildPostCommentPulse = 0;
#ifdef __GUILD_LEADER_SYSTEM__
	m_guildMemberGeneralPulse = 0;
#endif
}
...
void CGuild::Load(DWORD guild_id)
{
	...
	DBManager::instance().FuncQuery(std::bind1st(std::mem_fun(&CGuild::LoadGuildData), this),
		"SELECT master, level, exp, name, skill_point, skill, sp, ladder_point, win, draw, loss"
...
bool CGuild::OfferExp(LPCHARACTER ch, int amount)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ANTI_EXP_CAN_NOT_OFFER_GUILD_EXP"));
		return false;
	}
...
// @duzenleme comment uzerinden macro yaptiklari icin metin2dev uzerinden boyle bir fix aldik.
...
#ifdef __WIN32__
	DBManager::instance().FuncAfterQuery(std::bind(std::bind1st(std::mem_fun(&CGuild::RefreshCommentForce), this), ch->GetPlayerID()),
#else
	DBManager::instance().FuncAfterQuery(std::bind(std::bind1st(std::mem_fun(&CGuild::RefreshCommentForce), this), ch->GetPlayerID()),
#endif
...
bool CGuild::ChangeMemberGeneral(DWORD pid, BYTE is_general)
{
	...
	// Set comment pulse to 10 minutes
	m_guildMemberGeneralPulse = thecore_pulse() + PASSES_PER_SEC(10 * 60);
	return true;
}
