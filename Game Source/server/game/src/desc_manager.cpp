...
{
	if (test_server)
		sys_log(0, "BBBB ConnectAccount(%s)", login.c_str());
...
{
	if (test_server)
		sys_log(0, "BBBB DisConnectAccount(%s)", login.c_str());
...
struct name_with_desc_func
{
	...
	{
		// @duzenleme
		// karakterin ismini kuculterek ariyor cunku p2p uzerinde bulmuyor olabilir o yuzden eklendi..
