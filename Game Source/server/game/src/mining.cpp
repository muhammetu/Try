...
namespace mining
{
...
#ifdef __MARTY_PICKAXE_RENEWAL__
	...
	int RealRefinePick(LPCHARACTER ch, LPITEM item)
	{
		...
		if (Pick_IsRefineSuccess(rkOldPick))
		{
			...
			if (pkNewPick)
			{
				WORD bCell = rkOldPick.GetCell();
				rkItemMgr.RemoveItem(item, "REMOVE (REFINE PICK)");
				pkNewPick->__ADD_TO_CHARACTER(ch, TItemPos(INVENTORY, bCell));
				return 1;
			}
	...
	bool OreRefine(LPCHARACTER ch, LPCHARACTER npc, LPITEM item, int cost, int pct, LPITEM metinstone_item)
	{
		...
		long long llCost = ch->ComputeRefineFee(cost, 1);

		if (ch->GetGold() < llCost)
			return false;

		ch->PayRefineFee(llCost);
#else
